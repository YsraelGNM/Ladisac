...
Namespace Ladisac.Facturacion.Views
    Public Class frmRestriccionArticulo
#Region "Primaria"
        <Dependency()>
                ...
                Select Case vProceso
                    ...
                    Case "PrepararEliminar"
                        ...
                        Dim resp = Me.IBCBusqueda.RegistroAnterior("cast(" & Simple.CampoPrincipal & " as varchar) + " & _
                                                                   ...
                                                                   "convert(varchar," & Simple.CampoPrincipalTercero & ",103)", _
...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCRestriccionArticulo

        ' Controlar la clave de la tabla
        Public vCodigoART_ID As String = ""
        Public vCodigoREA_FECHA As String = ""

        ' CheckBox
        Private EchkREA_CONDICION As New chk
        Private EchkREA_ESTADO As New chk

        ' ComboBox
        Private EcboREA_MOVIMIENTO As New cbo
        ' TextBox
        '' PK
        ...
        '' Número
        Private EtxtREA_CANTIDAD As New txt

        Private Simple As New Ladisac.BE.RestriccionArticulo
        Private Simple1 As New Ladisac.BE.Personas
        Private Simple2 As New Ladisac.BE.Articulos

        Private ErrorData As New Ladisac.BE.RestriccionArticulo.ErrorData
        ...
        Private Sub LimpiarDatos()
            ...
            InicializarValores(chkREA_CONDICION, ErrorProvider1, False, False, EchkREA_CONDICION.pValorDefault)
            ColocarValoresDefault(chkREA_CONDICION)
            InicializarValores(txtREA_CANTIDAD, ErrorProvider1, EtxtREA_CANTIDAD.pSoloNumeros, EtxtREA_CANTIDAD.pSoloNumerosDecimales)
            InicializarValores(cboREA_MOVIMIENTO, ErrorProvider1)
            InicializarValores(chkREA_ESTADO, ErrorProvider1, False, False, EchkREA_ESTADO.pValorDefault)
            ColocarValoresDefault(chkREA_ESTADO)
        End Sub
        Private Sub HabilitarNuevo()
            txtART_ID.Enabled = True
            txtPER_ID.Enabled = True
            dtpREA_FECHA.Enabled = True
        End Sub
        ...
        Private Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "RegistroNoEncontrado"
                    ...
                    Simple.REA_FECHA = dtpREA_FECHA.Text.Trim
                ...
                Case "EliminarRegistro"
                    ...
                    Simple.REA_FECHA = dtpREA_FECHA.Text.Trim
                    vCodigoREA_FECHA = dtpREA_FECHA.Text.Trim
        ...
        Private Sub DatosCabecera()
            ...
            Simple.REA_FECHA = dtpREA_FECHA.value
            Simple.REA_CONDICION = DevolverTiposCampos(chkREA_CONDICION)
            Simple.REA_CANTIDAD = CDbl(txtREA_CANTIDAD.Text)
            Simple.REA_MOVIMIENTO = DevolverTiposCampos("REA_MOVIMIENTO", cboREA_MOVIMIENTO.Text, Simple)
            Simple.USU_ID = SessionService.UserId
            Simple.REA_FEC_GRAB = Now
            Simple.REA_ESTADO = DevolverTiposCampos(chkREA_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(dtpREA_FECHA, Simple.VerificarDatos("REA_FECHA"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkREA_CONDICION, Simple.VerificarDatos("REA_CONDICION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtREA_CANTIDAD, Simple.VerificarDatos("REA_CANTIDAD"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("REA_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkREA_ESTADO, Simple.VerificarDatos("REA_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.RestriccionArticulo
        End Sub
        ...
        Private Sub AdicionarElementoCombosEdicion()
            BuscarFormatos(EcboREA_MOVIMIENTO, Simple, cboREA_MOVIMIENTO, 0)
        End Sub
        Private Sub NombresFormulariosControles()
            EtxtPER_ID.pOOrm = Simple1
            EtxtPER_ID.pComportamiento = 1
            EtxtPER_ID.pOrdenBusqueda = 0
            EtxtART_ID.pOOrm = Simple2
...
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkREA_CONDICION = EchkTemporal
            EchkREA_CONDICION.pNombreCampo = "REA_CONDICION"
            EchkREA_CONDICION.pValorDefault = CheckState.Checked
            ConfigurarCheck_Refrescar(EchkREA_CONDICION)

            EchkREA_ESTADO = EchkTemporal
            EchkREA_ESTADO.pNombreCampo = "REA_ESTADO"
            EchkREA_ESTADO.pValorDefault = CheckState.Checked
            ConfigurarCheck_Refrescar(EchkREA_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkREA_CONDICION"
                    Simple.CampoId = EchkREA_CONDICION.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkREA_ESTADO"
                    Simple.CampoId = EchkREA_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                Case "chkREA_CONDICION"
                    vObjetoChk.pValorDefault = EchkREA_CONDICION.pValorDefault
                Case "chkREA_ESTADO"
                    vObjetoChk.pValorDefault = EchkREA_ESTADO.pValorDefault
            End Select
        ...
        Handles chkREA_CONDICION.CheckedChanged, _
        chkREA_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkREA_CONDICION"
                    If EchkREA_CONDICION.pFormatearTexto Then
                        InicializarTextoCheck(EchkREA_CONDICION)
                    End If
                Case "chkREA_ESTADO"
                    If EchkREA_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkREA_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            ...
            FormatearCampos(txtREA_CANTIDAD, "REA_CANTIDAD", EtxtREA_CANTIDAD, False)
            FormatearCampos(cboREA_MOVIMIENTO, "REA_MOVIMIENTO", Nothing)
        End Sub
...
#Region "ComboBox"
        Private Sub ConfigurarComboBox()
            EcboREA_MOVIMIENTO.pNombreCampo = "REA_MOVIMIENTO"
            cboREA_MOVIMIENTO.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList
        End Sub
#End Region
...
#Region "TextBox"
        ...
        txtREA_CANTIDAD.Validated
            Select Case sender.name.ToString
                Case "txtPER_ID"
                    ValidarDatos(EtxtPER_ID, txtPER_ID)
                Case "txtART_ID"
                    ValidarDatos(EtxtART_ID, txtART_ID)
                Case "txtREA_CANTIDAD"
                    ValidarDatos(EtxtREA_CANTIDAD, txtREA_CANTIDAD)
            End Select
        End Sub
        ...
        txtART_ID.DoubleClick
            ...
            Select Case sender.name.ToString
                ...
                Case "txtART_ID"
                    oDoubleClick(EtxtART_ID, txtART_ID, "frm")
            End Select
        End Sub
