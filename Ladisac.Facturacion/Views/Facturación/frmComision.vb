...
#Region "Secundaria 1"
        ...
        <Dependency()> _
        Public Property IBCComision As Ladisac.BL.IBCComision

        ' Check
        Private EchkCOM_ESTADO As New chk

        ' Combo
        Private EcboCOM_VENTA_CAN As New cbo

        ' Texto
        Private EtxtCOM_DESCRIPCION As New txt
        Private EtxtCOM_FORMULA As New txt

        ' Numeros
        Private EtxtCOM_POR_CUO_MEN As New txt
        Private EtxtCOM_POR_OBJ_MEN As New txt

        ' PK
        Private EtxtCOM_ID As New txt

        Private Simple As New Ladisac.BE.Comision
        Private ErrorData As New Ladisac.BE.Comision.ErrorData
...
#Region "Secundaria 2"
        Private Sub LimpiarDatos()
            ...
            InicializarValores(txtCOM_POR_CUO_MEN, ErrorProvider1, EtxtCOM_POR_CUO_MEN.pSoloNumeros, EtxtCOM_POR_CUO_MEN.pSoloNumerosDecimales)
            InicializarValores(txtCOM_POR_OBJ_MEN, ErrorProvider1, EtxtCOM_POR_OBJ_MEN.pSoloNumeros, EtxtCOM_POR_OBJ_MEN.pSoloNumerosDecimales)

            InicializarValores(cboCOM_VENTA_CAN, ErrorProvider1)
            InicializarValores(txtCOM_FORMULA, ErrorProvider1)

            InicializarValores(chkCOM_ESTADO, ErrorProvider1, False, False, EchkCOM_ESTADO.pValorDefault)
        End Sub
        Private Sub HabilitarNuevo()
            txtCOM_ID.Enabled = True
        End Sub

        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtCOM_ID)
        End Sub

        Private Sub HabilitarEscrituraNuevo()
            txtCOM_ID.ReadOnly = False
        End Sub
        ...
        Private Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "EliminarRegistro"
                    Simple.COM_ID = txtCOM_ID.Text.Trim
                    CodigoId = txtCOM_ID.Text.Trim
        ...
        Private Function Ingresar() As Boolean
            ...
            End If
            Simple = Nothing
            Simple = New Ladisac.BE.Comision
            Return Ingresar
        End Function

        Private Sub DatosCabecera()
            Simple.COM_ID = Strings.Trim(txtCOM_ID.Text)
            Simple.COM_DESCRIPCION = Strings.Trim(txtCOM_DESCRIPCION.Text)

            Simple.COM_POR_CUO_MEN = CDbl(txtCOM_POR_CUO_MEN.Text)
            Simple.COM_POR_OBJ_MEN = CDbl(txtCOM_POR_OBJ_MEN.Text)

            Simple.COM_VENTA_CAN = DevolverTiposCampos("COM_VENTA_CAN", cboCOM_VENTA_CAN.Text, Simple)

            Simple.COM_FORMULA = Strings.Trim(txtCOM_FORMULA.Text)

            Simple.USU_ID = SessionService.UserId
            Simple.COM_FEC_GRAB = Now
            Simple.COM_ESTADO = DevolverTiposCampos(chkCOM_ESTADO)
        End Sub
        ...
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtCOM_DESCRIPCION, Simple.VerificarDatos("COM_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCOM_POR_CUO_MEN, Simple.VerificarDatos("COM_POR_CUO_MEN"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCOM_POR_OBJ_MEN, Simple.VerificarDatos("COM_POR_OBJ_MEN"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(cboCOM_VENTA_CAN, Simple.VerificarDatos("COM_VENTA_CAN"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCOM_FORMULA, Simple.VerificarDatos("COM_FORMULA"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("COM_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkCOM_ESTADO, Simple.VerificarDatos("COM_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Function InsertarDatos() As Boolean
            ...
            Simple.MarkAsAdded()
            vRespuestaLocal = Me.IBCComision.MantenimientoComision(Simple)
        ...
        Private Function ActualizarDatos() As Boolean
            ...
            ActualizarDatos = (Me.IBCComision.MantenimientoComision(Simple) > 0)
        End Function
        ...
        Private Function EliminarRegistro() As Boolean
            ...
            If (Me.IBCComision.MantenimientoComision(Simple) > 0) Then
        ...
        Private Sub frm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            ...
            Try
                ...
                FormatearCampos(txtCOM_DESCRIPCION, "COM_DESCRIPCION", EtxtCOM_DESCRIPCION)
                FormatearCampos(txtCOM_POR_CUO_MEN, "COM_POR_CUO_MEN", EtxtCOM_POR_CUO_MEN, False)
                FormatearCampos(txtCOM_POR_OBJ_MEN, "COM_POR_OBJ_MEN", EtxtCOM_POR_OBJ_MEN, False)
                FormatearCampos(cboCOM_VENTA_CAN, "COM_VENTA_CAN", Nothing)
                FormatearCampos(txtCOM_FORMULA, "COM_FORMULA", EtxtCOM_FORMULA, False)
        ...
        Private Sub ConfigurarCheck()
            ...
            EchkCOM_ESTADO.pValorDefault = CheckState.Checked
        ...
        Private Sub ValoresDefaultNuevo()
            ...
            EchkCOM_ESTADO.pValorDefault = CheckState.Unchecked
        End Sub
        ...
        Private Sub ConfigurarComboBox()
            EcboCOM_VENTA_CAN.pNombreCampo = "COM_VENTA_CAN"
            cboCOM_VENTA_CAN.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList
        End Sub
        Private Sub ConfigurarText()
            ...
            EtxtCOM_DESCRIPCION = EtxtTemporal
            EtxtCOM_FORMULA = EtxtTemporal
            EtxtCOM_POR_CUO_MEN = EtxtTemporal
            EtxtCOM_POR_OBJ_MEN = EtxtTemporal
        End Sub
        Private Sub AdicionarElementoCombosEdicion()
            BuscarFormatos(EcboCOM_VENTA_CAN, Simple, cboCOM_VENTA_CAN, 0)
        End Sub
...
#Region "Check"
        Private Sub chk_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles chkCOM_ESTADO.CheckedChanged
...
#Region "Text"
        Private Sub txt_OnGotFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
            Handles txtCOM_DESCRIPCION.GotFocus, txtCOM_FORMULA.GotFocus, _
                    txtCOM_POR_CUO_MEN.GotFocus, txtCOM_POR_OBJ_MEN.GotFocus
            Select Case sender.name.ToString
            End Select
        End Sub
        Private Sub txt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
            Handles txtCOM_DESCRIPCION.LostFocus, txtCOM_FORMULA.LostFocus, _
                    txtCOM_POR_CUO_MEN.LostFocus, txtCOM_POR_OBJ_MEN.LostFocus
            Select Case sender.name.ToString
            End Select
        End Sub
        Private Sub txt_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles txtCOM_DESCRIPCION.Validated, txtCOM_FORMULA.Validated, _
                    txtCOM_POR_CUO_MEN.Validated, txtCOM_POR_OBJ_MEN.Validated

            Select Case sender.name.ToString
                Case "txtCOM_DESCRIPCION"
                    ValidarDatos(EtxtCOM_DESCRIPCION, txtCOM_DESCRIPCION)
                Case "txtCOM_FORMULA"
                    ValidarDatos(EtxtCOM_FORMULA, txtCOM_FORMULA)
                Case "txtCOM_POR_CUO_MEN"
                    ValidarDatos(EtxtCOM_POR_CUO_MEN, txtCOM_POR_CUO_MEN)
                Case "txtCOM_POR_OBJ_MEN"
                    ValidarDatos(EtxtCOM_POR_OBJ_MEN, txtCOM_POR_OBJ_MEN)
            End Select
        End Sub
        Private Sub txt_OnKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) _
            Handles txtCOM_DESCRIPCION.KeyDown, txtCOM_FORMULA.KeyDown, _
                    txtCOM_POR_CUO_MEN.KeyDown, txtCOM_POR_OBJ_MEN.KeyDown
        ...
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
            Handles txtCOM_DESCRIPCION.KeyPress, txtCOM_FORMULA.KeyPress, _
                    txtCOM_POR_CUO_MEN.KeyPress, txtCOM_POR_OBJ_MEN.KeyPress

            Select Case sender.name.ToString
                Case "txtCOM_DESCRIPCION"
                    oKeyPress(EtxtCOM_DESCRIPCION, e)
                Case "txtCOM_FORMULA"
                    oKeyPress(EtxtCOM_FORMULA, e)
                Case "txtCOM_POR_CUO_MEN"
                    oKeyPress(EtxtCOM_POR_CUO_MEN, e)
                Case "txtCOM_POR_OBJ_MEN"
                    oKeyPress(EtxtCOM_POR_OBJ_MEN, e)
            End Select
        End Sub
        ...
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
              Handles txtCOM_DESCRIPCION.DoubleClick, txtCOM_FORMULA.DoubleClick, _
                      txtCOM_POR_CUO_MEN.DoubleClick, txtCOM_POR_OBJ_MEN.DoubleClick
