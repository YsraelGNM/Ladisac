...
Namespace Ladisac.Facturacion.Views
    Public Class frmGenerarDocumentoPromocion
#Region "Primaria"
        <Dependency()>
...
#End Region
        ...
        Private Function InsertarDatos() As Boolean
            ...
            vRespuestaLocal = Me.IBC.spInsertarRegistro(Compuesto)
        ...
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkDPR_ESTADO"
                    Compuesto.CampoId = EchkDPR_ESTADO.pNombreCampo
        ...
        Private Sub BuscarFormatos(ByRef vObjeto As cbo, ByVal oCompuesto As Object)
            ...
            Select Case vObjeto.pNombreCampo
                Case "DPR_TIPO_PROMOCION"
                    cMisProcedimientos.AdicionarElementoCombosEdicion(cboDPR_TIPO_PROMOCION, oCompuesto.BuscarFormatos(), 0)
            End Select
        End Sub
...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCDocumentosPromocion

        <Dependency()> _
        Public Property IBCDetalle As Ladisac.BL.IBCDetalleDocumentosPromocion
        ...
        Private EchkPER_ESTADO_PRO As New chk
        Private EchkDPR_ESTADO As New chk

        Private EcboDPR_TIPO_PROMOCION As New cbo

        Private EdgvDetalle As New dgv

        Private EtxtPER_ID_PRO As New txt

        Private Compuesto As New Ladisac.BE.DocumentosPromocion
        Private CompuestoObjeto1 As New Ladisac.BE.DetalleDocumentosPromocion
        Private ErrorData As New Ladisac.BE.DocumentosPromocion.ErrorData
        Private cMisProcedimientos As New Ladisac.MisProcedimientos

        Private Structure ElementosEliminar
            Public cDPR_NUMERO As String
            Public cDPR_TIPO_PROMOCION As String
            Public cDDP_ITEM As String
        End Structure
        ...
        Private Sub LimpiarDatos()
            ...
            txtDPR_NUMERO.Text = ""
            ErrorProvider1.SetError(txtDPR_NUMERO, Nothing)

            cboDPR_TIPO_PROMOCION.Text = ""
            ErrorProvider1.SetError(cboDPR_TIPO_PROMOCION, Nothing)

            txtPER_ID_PRO.Text = ""
            ErrorProvider1.SetError(txtPER_ID_PRO, Nothing)

            txtPER_DESCRIPCION_PRO.Text = ""
            ErrorProvider1.SetError(txtPER_DESCRIPCION_PRO, Nothing)

            chkPER_ESTADO_PRO.Checked = True
            chkPER_ESTADO_PRO.CheckState = EchkPER_ESTADO_PRO.pValorDefault
            ErrorProvider1.SetError(chkPER_ESTADO_PRO, Nothing)


            chkDPR_ESTADO.Checked = True
            chkDPR_ESTADO.CheckState = EchkDPR_ESTADO.pValorDefault
            ErrorProvider1.SetError(chkDPR_ESTADO, Nothing)
        ...
        Private Sub HabilitarNuevo()
            txtDPR_NUMERO.Enabled = True
            cboDPR_TIPO_PROMOCION.Enabled = True
            txtPER_ID_PRO.Enabled = True
        End Sub
        Private Sub ValoresDefaultNuevo()
            ColocarValoresDefault(chkPER_ESTADO_PRO)
            ColocarValoresDefault(chkDPR_ESTADO)
        End Sub

        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtDPR_NUMERO)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtDPR_NUMERO.ReadOnly = False
            cboDPR_TIPO_PROMOCION.Enabled = False
            txtPER_ID_PRO.ReadOnly = False
        End Sub
                ...
                Try
                    With dgvDetalle.Rows(dgvDetalle.CurrentRow.Index)
                        If .Cells("cEstadoRegistro").Value Then
                            eRegistrosEliminar(eRegistrosEliminar.Count() - 1).cDPR_NUMERO = txtDPR_NUMERO.Text
                            eRegistrosEliminar(eRegistrosEliminar.Count() - 1).cDPR_TIPO_PROMOCION = DevolverTiposCampos("DPR_TIPO_PROMOCION", cboDPR_TIPO_PROMOCION.Text, Compuesto)
                            eRegistrosEliminar(eRegistrosEliminar.Count() - 1).cDDP_ITEM = .Cells("cDDP_ITEM").Value.ToString()
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "EliminarRegistro"
                    Compuesto.DPR_NUMERO = txtDPR_NUMERO.Text.Trim
                    CodigoId = txtDPR_NUMERO.Text.Trim
                Case "InicializarDatos"
                    Compuesto.DPR_NUMERO = txtDPR_NUMERO.Text.Trim
                    CompuestoObjeto1.DDP_NUMERO = txtDPR_NUMERO.Text.Trim
                    CodigoId = txtDPR_NUMERO.Text.Trim
                    ...
                    If vBuscarDetalle Then
                        ...
                        Dim sr As New StringReader(IBCMaestro.EjecutarVista(NombreProcedimiento, CodigoId, cboDPR_TIPO_PROMOCION.Text, "  "))
        ...
        Private Sub DatosCabecera()
            Compuesto.DPR_NUMERO = Strings.Trim(txtDPR_NUMERO.Text)
            Compuesto.DPR_TIPO_PROMOCION = DevolverTiposCampos("DPR_TIPO_PROMOCION", cboDPR_TIPO_PROMOCION.Text, Compuesto)
            Compuesto.DPR_FECHA = dtpDPR_FECHA.Value
            ...
            Compuesto.USU_ID = SessionService.UserId
            Compuesto.DPR_FEC_GRAB = Now
            Compuesto.DPR_ESTADO = DevolverTiposCampos(chkDPR_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    resp.rM = Compuesto.ColocarErrores(txtDPR_NUMERO, Compuesto.VerificarDatos("LPR_ID"), ErrorProvider1)
                    resp.rM = Compuesto.ColocarErrores(cboDPR_TIPO_PROMOCION, Compuesto.VerificarDatos("LRP_PRINCIPAL"), ErrorProvider1)
                    resp.rM = Compuesto.ColocarErrores(dtpDPR_FECHA, Compuesto.VerificarDatos("DPR_FECHA"), ErrorProvider1)
                    resp.rM = Compuesto.ColocarErrores(txtPER_ID_PRO, Compuesto.VerificarDatos("PER_ID"), ErrorProvider1)
                    resp.rM = Compuesto.ColocarErrores(pnCuerpo, Compuesto.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Compuesto.ColocarErrores(btnImagen, Compuesto.VerificarDatos("DPR_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Compuesto.ColocarErrores(chkDPR_ESTADO, Compuesto.VerificarDatos("DPR_ESTADO"), ErrorProvider1)
                Case "Detalle"
                    resp.rM = CompuestoObjeto1.ColocarErrores(dgvDetalle, _
                                                       CompuestoObjeto1.VerificarDatos("DDP_NUMERO", _
                                                                                       "DDP_TIPO_PROMOCION", _
                                                                                       "DDP_ITEM", _
                                                                                       "TDO_ID_DOC", _
                                                                                       "DTD_ID_DOC", _
                                                                                       "CCT_ID_DOC", _
        ...
        Private Sub LimpiarGrilla()
            dgvDetalle.Rows.Clear()
            'If Trim(txtPER_ID_PRO.Text) <> "" Then txtLPR_DESCRIPCION.Text = Strings.LSet((txtPER_DESCRIPCION_PRO.Text & " - " & cboDPR_TIPO_PROMOCION.Text), LongitudCampoTabla("LPR_DESCRIPCION", Compuesto.vArrayDatosComboBox, Compuesto.vElementosDatosComboBox - 1))
        End Sub
        ...
        Private Sub frm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            Try
                ...
                FormatearCampos(txtDPR_NUMERO, "DPR_NUMERO")
                FormatearCampos(cboDPR_TIPO_PROMOCION, "DPR_TIPO_PROMOCION")
                FormatearCampos(txtPER_ID_PRO, "PER_ID_PRO")
            Catch ex As Exception
                MsgBox(Err.Number & " - " & ex.Message, MsgBoxStyle.Information, Me.Text & " - Load")
            End Try
        End Sub
        Private Sub AdicionarElementoCombosEdicion()
            BuscarFormatos(EcboDPR_TIPO_PROMOCION, Compuesto)
        End Sub
        Private Sub NombresFormulariosControles()
            EtxtPER_ID_PRO.pOOrm = New Ladisac.BE.Personas
            EtxtPER_ID_PRO.pComportamiento = 1
            EtxtPER_ID_PRO.pOrdenBusqueda = 0
            If cboDPR_TIPO_PROMOCION.Text = "MAESTRO CONSTRUCTOR" Then
                EtxtPER_ID_PRO.pOOrm.CadenaFiltrado = " and PER_CONTACTO='SI'"
            ElseIf cboDPR_TIPO_PROMOCION.Text = "PROMOCION ECO DIAMANTE" Then
                EtxtPER_ID_PRO.pOOrm.CadenaFiltrado = " and PER_CONTACTO='SI' AND PER_ID in (select per_id from mae.personaspromocion where tipo_promocion=1 and pep_estado=1)"
            End If
        End Sub
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            EchkPER_ESTADO_PRO.pFormatearTexto = True
            EchkPER_ESTADO_PRO.pNombreCampo = "PER_ESTADO_PRO"
            EchkPER_ESTADO_PRO.pSimple = Compuesto
            EchkPER_ESTADO_PRO.pValorDefault = CheckState.Indeterminate
            EchkPER_ESTADO_PRO.vEstado0 = ""
            EchkPER_ESTADO_PRO.vEstado1 = ""
            EchkPER_ESTADO_PRO.vEstadoX = ""

            EchkDPR_ESTADO.pFormatearTexto = True
            EchkDPR_ESTADO.pNombreCampo = "DPR_ESTADO"
            EchkDPR_ESTADO.pSimple = Compuesto
            EchkDPR_ESTADO.pValorDefault = CheckState.Checked
            EchkDPR_ESTADO.vEstado0 = ""
            EchkDPR_ESTADO.vEstado1 = ""
            EchkDPR_ESTADO.vEstadoX = ""

            ConfigurarCheck_Refrescar(EchkPER_ESTADO_PRO)
            ConfigurarCheck_Refrescar(EchkDPR_ESTADO)
        End Sub
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                Case "chkPER_ESTADO_PRO"
                    vObjetoChk.pValorDefault = EchkPER_ESTADO_PRO.pValorDefault
                Case "chkDPR_ESTADO"
                    vObjetoChk.pValorDefault = EchkDPR_ESTADO.pValorDefault
            End Select
        ...
        Private Sub chk_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles chkDPR_ESTADO.CheckedChanged, chkPER_ESTADO_PRO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkPER_ESTADO_PRO"
                    If EchkPER_ESTADO_PRO.pFormatearTexto Then
                        InicializarTexto(EchkPER_ESTADO_PRO)
                    End If
                Case "chkDPR_ESTADO"
                    If EchkDPR_ESTADO.pFormatearTexto Then
                        InicializarTexto(EchkDPR_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "DataGridView"
        ...
        Private Sub dgvDetalle_CellValidated(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) _
            ...
            Select Case sender.Columns(dgvDetalle.CurrentCell.ColumnIndex).Name.ToString
                Case "cDDP_PUNTOS"
                    If dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value > dgvDetalle.Item("cDDP_PUNTOS_CONTROL", dgvDetalle.CurrentRow.Index).Value Then
                        dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value = dgvDetalle.Item("cDDP_PUNTOS_CONTROL", dgvDetalle.CurrentRow.Index).Value
                    End If
...
#Region "TextBox"
        Private Sub ConfigurarText()
            EtxtPER_ID_PRO.pTexto1 = ""
            EtxtPER_ID_PRO.pTexto2 = ""
            EtxtPER_ID_PRO.pSoloNumerosDecimales = False
            EtxtPER_ID_PRO.pSoloNumeros = False
            EtxtPER_ID_PRO.pParteEntera = 0
            EtxtPER_ID_PRO.pParteDecimal = 0
            EtxtPER_ID_PRO.pMinusculaMayuscula = True
            EtxtPER_ID_PRO.pBusqueda = True
            EtxtPER_ID_PRO.pCadenaFiltrado = ""

            EtxtPER_ID_PRO.pOOrm = New Ladisac.BE.Personas
            EtxtPER_ID_PRO.pFormularioConsulta = True
        End Sub
#End Region
...
#Region "ComboBox"
        Private Sub ConfigurarComboBox()
            EcboDPR_TIPO_PROMOCION.pNombreCampo = "DPR_TIPO_PROMOCION"
            cboDPR_TIPO_PROMOCION.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList
        End Sub
#End Region
#Region "TextBox"
        Private Sub txt_OnGotFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
        Handles txtPER_ID_PRO.GotFocus
            Select Case sender.name.ToString
                Case "txtPER_ID_PRO"
        ...
        Handles txtPER_ID_PRO.LostFocus
            Select Case sender.name.ToString
                Case "txtPER_ID_PRO"
        ...
        Handles txtPER_ID_PRO.Validated
            Select Case sender.name.ToString
                Case "txtPER_ID_PRO"
                    txtPER_ID_PRO.Text = cMisProcedimientos.VerificarLongitud(txtPER_ID_PRO.Text, txtPER_ID_PRO.MaxLength)
        ...
        Handles txtPER_ID_PRO.KeyPress
            Select Case sender.name.ToString
                Case "txtPER_ID_PRO"
        ...
        Handles txtPER_ID_PRO.DoubleClick
            Select Case sender.name.ToString
                Case "txtPER_ID_PRO"
...
#End Region
        ...
        Private Function EliminarRegistroDetalle() As Int16
            ...
            Else
                ...
                For fila = 1 To eRegistrosEliminar.Count - 2
                    ...
                    'CompuestoObjeto1.DDP_NUMERO = txtDPR_NUMERO.Text
                    'CompuestoObjeto1.DDP_TIPO_PROMOCION = DevolverTiposCampos("DPR_TIPO_PROMOCION", cboDPR_TIPO_PROMOCION.Text, Compuesto)
                    'CompuestoObjeto1.DDP_ITEM = .Cells("cDDP_ITEM").Value
                    CompuestoObjeto1.USU_ID = SessionService.UserId

                    EliminarRegistroDetalle = Me.IBCDetalle.spEliminarRegistro(CompuestoObjeto1)
        ...
        Private Function ProcesarDatosDetalle() As Int16
            ...
            While (dgvDetalle.Rows.Count() > vFilGrid)
                With dgvDetalle.Rows(vFilGrid)
                    ...
                    CompuestoObjeto1.DDP_TIPO_PROMOCION = DevolverTiposCampos("DPR_TIPO_PROMOCION", cboDPR_TIPO_PROMOCION.Text, Compuesto)
                    CompuestoObjeto1.DDP_ITEM = .Cells("cDDP_ITEM").Value
                    CompuestoObjeto1.TDO_ID_DOC = .Cells("cTDO_ID_DOC").Value
                    CompuestoObjeto1.DTD_ID_DOC = .Cells("cDTD_ID_DOC").Value
                    CompuestoObjeto1.CCT_ID_DOC = .Cells("cCCT_ID_DOC").Value
                    CompuestoObjeto1.DDP_SERIE_DOC = .Cells("cDDP_SERIE_DOC").Value
                    CompuestoObjeto1.DDP_NUMERO_DOC = .Cells("cDDP_NUMERO_DOC").Value
                    CompuestoObjeto1.ART_ID = .Cells("cART_ID").Value
                    CompuestoObjeto1.DDP_PUNTOS = .Cells("cDDP_PUNTOS").Value
                    CompuestoObjeto1.DDP_PUNTOS_CONTROL = .Cells("cDDP_PUNTOS_CONTROL").Value
                    CompuestoObjeto1.USU_ID = SessionService.UserId
                    CompuestoObjeto1.DDP_FEC_GRAB = Now
                    CompuestoObjeto1.DDP_ESTADO = DevolverTiposCampos("DDP_ESTADO", .Cells("cDDP_ESTADO").Value.ToString(), CompuestoObjeto1)
                    ...
                    If (.Cells("cEstadoRegistro").Value = 1 Or .Cells("cEstadoRegistro").Value = True) Then
                        ProcesarDatosDetalle = Me.IBCDetalle.spActualizarRegistro(CompuestoObjeto1)
                    ...
                    Else
                        ...
                        ProcesarDatosDetalle = Me.IBCDetalle.spInsertarRegistro(CompuestoObjeto1)
                        If ProcesarDatosDetalle = 0 Then
                            vMensajeErrorOrm = CompuestoObjeto1.vMensajeError
                            Exit Function
                        End If
        ...
        End Function
        Private Function EliminarDetalle(ByVal oOrm As DetalleDocumentosPromocion) As Int16
            EliminarDetalle = Me.IBCDetalle.spEliminarRegistroGeneral(oOrm)
        ...
        Private Sub cboDPR_TIPO_PROMOCION_SelectedValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboDPR_TIPO_PROMOCION.SelectedValueChanged
            If sender.TEXT = "MAESTRO CONSTRUCTOR" Then
                EtxtPER_ID_PRO.pOOrm.CadenaFiltrado = " and PER_CONTACTO='SI'"
            ElseIf sender.TEXT = "PROMOCION ECO DIAMANTE" Then
                EtxtPER_ID_PRO.pOOrm.CadenaFiltrado = " and PER_CONTACTO='SI' AND PER_ID in (select per_id from mae.personaspromocion where tipo_promocion=1 and pep_estado=1)"
            End If
            txtPER_ID_PRO.Text = ""
            txtPER_DESCRIPCION_PRO.Text = ""
        ...
        Private Sub btnJalarDatos_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnJalarDatos.Click
            If Trim(txtPER_ID_PRO.Text) = "" Then
                MsgBox("¡Debe ingresar el código del promotor!", MsgBoxStyle.Information, Me.Text)
                Exit Sub
            End If
            ...
            NombreProcedimiento = Ladisac.DA.SPNames.spDocumentosBVFTConPromotorEscogerXML 'Ladisac.DA.SPNames.spDocumentosBVFTConPromotorXML

            vPVE_ID = DBNull.Value
            vPER_ID_CLI = txtPER_ID_PRO.Text


            If cboDPR_TIPO_PROMOCION.Text = "MAESTRO CONSTRUCTOR" Then
                vFiltroProcedimiento &= " WHERE PER_ID_PRO = '" & vPER_ID_CLI & "'"
            ...
            ElseIf cboDPR_TIPO_PROMOCION.Text = "PROMOCION ECO DIAMANTE" Then
                vFiltroProcedimiento &= " WHERE PER_ID_PRO = '" & vPER_ID_CLI & "'"
                vFiltroProcedimiento &= " And DTD_ID  In ('" & BCVariablesNames.ProcesosFacturación.Boleta & "'," & _
                                                         "'" & BCVariablesNames.ProcesosFacturación.Factura & "')  AND art_id_imp in ('020276') "
            End If
        ...
        Private Sub dtpDPR_FECHA_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dtpDPR_FECHA.ValueChanged
            LimpiarGrilla()
        End Sub
    End Class
End Namespace
