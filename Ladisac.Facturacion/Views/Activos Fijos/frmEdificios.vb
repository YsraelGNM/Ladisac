...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCEdificios
        ...
        '' Texto
        Private EtxtEDI_DESCRIPCION As New txt

        '' Número

        Private Simple As New Ladisac.BE.Edificios
        Private Simple1 As New Ladisac.BE.PuntoVenta
        Private ErrorData As New Ladisac.BE.Edificios.ErrorData
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtEDI_ID)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtEDI_ID.ReadOnly = False
        End Sub
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "PrepararEliminar"
                    ...
                    Simple.EDI_ID = CodigoId
                Case "Load"
                    Simple.Vista = "PrimerAnterior"
                    Simple.EDI_ID = CodigoId
                Case "RegistroNoEncontrado"
                    Simple.EDI_ID = txtEDI_ID.Text.Trim
                Case "NavegarFormulario"
                    CodigoId = CodigoId
                Case "EliminarRegistro"
                    Simple.EDI_ID = txtEDI_ID.Text.Trim
                    CodigoId = txtEDI_ID.Text.Trim
        ...
        Private Sub DatosCabecera()
            ...
            Simple.EDI_DESCRIPCION = Strings.Trim(txtEDI_DESCRIPCION.Text)
            Simple.PVE_ID = Strings.Trim(txtPVE_ID.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.EDI_FEC_GRAB = Now
            Simple.EDI_ESTADO = DevolverTiposCampos(chkEDI_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtEDI_DESCRIPCION, Simple.VerificarDatos("EDI_DESCRIPCION"), ErrorProvider1)
                    ...
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("EDI_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkEDI_ESTADO, Simple.VerificarDatos("EDI_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.Edificios
        End Sub
...
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkEDI_ESTADO.pValorDefault = CheckState.Checked
        ...
        Handles chkEDI_ESTADO.CheckedChanged
...
#Region "TextBox"
        Private Sub ConfigurarText()
            ...
            EtxtEDI_DESCRIPCION = EtxtTemporal
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            ...
            FormatearCampos(txtEDI_DESCRIPCION, "EDI_DESCRIPCION", EtxtEDI_DESCRIPCION)
            FormatearCampos(txtPVE_ID, "PVE_ID", EtxtPVE_ID)
            FormatearCampos(txtPVE_DESCRIPCION, "PVE_DESCRIPCION", Nothing)
        End Sub
...
#Region "TextBox"
        ...
        Handles txtEDI_ID.Validated, _
                txtEDI_DESCRIPCION.Validated, _
                txtPVE_ID.Validated
            Select Case sender.name.ToString
                Case "txtEDI_ID"
                    ValidarDatos(EtxtEDI_ID, txtEDI_ID)
                Case "txtEDI_DESCRIPCION"
                    ValidarDatos(EtxtEDI_DESCRIPCION, txtEDI_DESCRIPCION)
                Case "txtPVE_ID"
        ...
        Handles txtEDI_ID.KeyPress, _
                txtEDI_DESCRIPCION.KeyPress, _
                txtPVE_ID.KeyPress
            Select Case sender.name.ToString
                Case "txtEDI_ID"
                    oKeyPress(EtxtEDI_ID, e)
                Case "txtEDI_DESCRIPCION"
                    oKeyPress(EtxtEDI_DESCRIPCION, e)
                Case "txtPVE_ID"
