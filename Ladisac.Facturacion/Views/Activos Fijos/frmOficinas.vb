...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCOficinas

        ' Controlar la clave de la tabla

        ' CheckBox
        Private EchkEDI_ESTADO As New chk
        ...
        Private EtxtEDI_ID As New txt
        '' Texto
        Private EtxtOFI_DESCRIPCION As New txt

        '' Número

        Private Simple As New Ladisac.BE.Oficinas
        Private Simple1 As New Ladisac.BE.Edificios
        Private ErrorData As New Ladisac.BE.Oficinas.ErrorData
        ...
        Private Sub LimpiarDatos()
            ...
            InicializarValores(txtEDI_ID, ErrorProvider1)
            InicializarValores(txtEDI_DESCRIPCION, ErrorProvider1)
            InicializarValores(chkEDI_ESTADO, ErrorProvider1, False, False, EchkEDI_ESTADO.pValorDefault)
            ColocarValoresDefault(chkEDI_ESTADO)
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtOFI_ID)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtOFI_ID.ReadOnly = False
        End Sub
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "PrepararEliminar"
                    ...
                    Simple.OFI_ID = CodigoId
                Case "Load"
                    Simple.Vista = "PrimerAnterior"
                    Simple.OFI_ID = CodigoId
                Case "RegistroNoEncontrado"
                    Simple.OFI_ID = txtOFI_ID.Text.Trim
                Case "NavegarFormulario"
                    CodigoId = CodigoId
                Case "EliminarRegistro"
                    Simple.OFI_ID = txtOFI_ID.Text.Trim
                    CodigoId = txtOFI_ID.Text.Trim
        ...
        Private Sub DatosCabecera()
            ...
            Simple.OFI_DESCRIPCION = Strings.Trim(txtOFI_DESCRIPCION.Text)
            Simple.EDI_ID = Strings.Trim(txtEDI_ID.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.OFI_FEC_GRAB = Now
            Simple.OFI_ESTADO = DevolverTiposCampos(chkOFI_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtOFI_DESCRIPCION, Simple.VerificarDatos("OFI_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtEDI_ID, Simple.VerificarDatos("EDI_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("OFI_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkOFI_ESTADO, Simple.VerificarDatos("OFI_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.Oficinas
        End Sub
        Public Sub FiltrarCampos(ByVal vComportamiento As Integer)
        End Sub
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                If Me.ActiveControl.Name.ToString = "txtEDI_ID" Then EtxtEDI_ID.pTexto2 = Me.ActiveControl.Text
        ...
        Private Sub NombresFormulariosControles()
            EtxtEDI_ID.pOOrm = Simple1
            EtxtEDI_ID.pComportamiento = 1
            EtxtEDI_ID.pOrdenBusqueda = 0
        End Sub
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkEDI_ESTADO = EchkTemporal
            EchkEDI_ESTADO.pNombreCampo = "EDI_ESTADO"

            EchkOFI_ESTADO = EchkTemporal
            EchkOFI_ESTADO.pNombreCampo = "OFI_ESTADO"
            EchkOFI_ESTADO.pValorDefault = CheckState.Checked

            ConfigurarCheck_Refrescar(EchkEDI_ESTADO)
        ...
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkEDI_ESTADO"
                    Simple.CampoId = EchkEDI_ESTADO.pNombreCampo
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                Case "chkEDI_ESTADO"
                    vObjetoChk.pValorDefault = EchkEDI_ESTADO.pValorDefault
                Case "chkOFI_ESTADO"
        ...
        Handles chkOFI_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkEDI_ESTADO"
                    If EchkEDI_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkEDI_ESTADO)
                    End If
                Case "chkOFI_ESTADO"
...
#Region "TextBox"
        Private Sub ConfigurarText()
            ...
            EtxtOFI_DESCRIPCION = EtxtTemporal
            EtxtEDI_ID = EtxtTemporal

            EtxtEDI_ID.pBusqueda = True
            EtxtEDI_ID.pFormularioConsulta = True

        End Sub
        Private Sub txt_OnKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) _
        Handles txtEDI_ID.KeyDown
            Select Case e.KeyCode
                Case Keys.F1
                    Select Case sender.name.ToString
                        Case "txtEDI_ID"
                            TeclaF1(EtxtEDI_ID, txtEDI_ID)
                    End Select
            End Select
        End Sub
#End Region
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            ...
            FormatearCampos(txtOFI_DESCRIPCION, "OFI_DESCRIPCION", EtxtOFI_DESCRIPCION)
            FormatearCampos(txtEDI_ID, "EDI_ID", EtxtEDI_ID)
            FormatearCampos(txtEDI_DESCRIPCION, "EDI_DESCRIPCION", Nothing)
        End Sub
        ...
        Private Sub FiltrarOrm()
            EtxtEDI_ID.pOOrm.CadenaFiltrado = ""
        End Sub
#Region "ComboBox"
...
#Region "TextBox"
        ...
        Handles txtEDI_ID.GotFocus
            Select Case sender.name.ToString
                Case "txtEDI_ID"
                    EtxtEDI_ID.pTexto1 = sender.text
            End Select
        End Sub
        Private Sub txt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
        Handles txtEDI_ID.LostFocus
            Select Case sender.name.ToString
                Case "txtEDI_ID"
                    EtxtEDI_ID.pTexto2 = sender.text
            End Select
        End Sub
        Private Sub txt_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtOFI_ID.Validated, _
                txtOFI_DESCRIPCION.Validated, _
                txtEDI_ID.Validated
            Select Case sender.name.ToString
                Case "txtOFI_ID"
                    ValidarDatos(EtxtOFI_ID, txtOFI_ID)
                Case "txtOFI_DESCRIPCION"
                    ValidarDatos(EtxtOFI_DESCRIPCION, txtOFI_DESCRIPCION)
                Case "txtEDI_ID"
                    ValidarDatos(EtxtEDI_ID, txtEDI_ID)
            End Select
        End Sub
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
        Handles txtOFI_ID.KeyPress, _
        txtOFI_DESCRIPCION.KeyPress, _
        txtEDI_ID.KeyPress
            Select Case sender.name.ToString
                Case "txtOFI_ID"
                    oKeyPress(EtxtOFI_ID, e)
                Case "txtOFI_DESCRIPCION"
                    oKeyPress(EtxtOFI_DESCRIPCION, e)
                Case "txtEDI_ID"
                    oKeyPress(EtxtEDI_ID, e)
            End Select
        End Sub
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtEDI_ID.DoubleClick
            Select Case sender.name.ToString
                Case "txtEDI_ID"
                    oDoubleClick(EtxtEDI_ID, txtEDI_ID, "frmEdificios")
            End Select
        End Sub
        Private Sub oDoubleClick(ByVal EtxtTemporal As txt, ByRef txt As TextBox, ByVal e As System.String)
            ...
            If EtxtTemporal.pFormularioConsulta Then
                Select Case e
                    Case "frmEdificios"
                        frmconsulta = Me.ContainerService.Resolve(Of Ladisac.ActivosFijos.Views.frmEdificios)()
                    Case Else
                        Exit Sub
                End Select
