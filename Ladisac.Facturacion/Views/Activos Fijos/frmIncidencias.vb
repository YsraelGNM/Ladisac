...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCIncidencias

        ' Controlar la clave de la tabla

        ' CheckBox
        Private EchkINC_ESTADO As New chk

        ' ComboBox
        Private EcboINC_TIPO As New cbo


        ' TextBox
        '' PK
        Private EtxtINC_ID As New txt
        '' Texto
        Private EtxtINC_DESCRIPCION As New txt

        '' Número

        Private Simple As New Ladisac.BE.Incidencias
        Private ErrorData As New Ladisac.BE.Incidencias.ErrorData
        Private cMisProcedimientos As New Ladisac.MisProcedimientos

        Private Sub LimpiarDatos()
            InicializarValores(txtINC_ID, ErrorProvider1)
            InicializarValores(txtINC_DESCRIPCION, ErrorProvider1)
            InicializarValores(cboINC_TIPO, ErrorProvider1)
            InicializarValores(chkINC_ESTADO, ErrorProvider1, False, False, EchkINC_ESTADO.pValorDefault)
            ColocarValoresDefault(chkINC_ESTADO)
        End Sub
        Private Sub HabilitarNuevo()
            txtINC_ID.Enabled = True
        End Sub
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtINC_ID)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtINC_ID.ReadOnly = False
        End Sub
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "PrepararEliminar"
                    ...
                    Simple.INC_ID = CodigoId
                Case "Load"
                    Simple.Vista = "PrimerAnterior"
                    Simple.INC_ID = CodigoId
                Case "RegistroNoEncontrado"
                    Simple.INC_ID = txtINC_ID.Text.Trim
                Case "NavegarFormulario"
                    CodigoId = CodigoId
                Case "EliminarRegistro"
                    Simple.INC_ID = txtINC_ID.Text.Trim
                    CodigoId = txtINC_ID.Text.Trim
        ...
        Private Sub DatosCabecera()
            Simple.INC_ID = Strings.Trim(txtINC_ID.Text)
            Simple.INC_DESCRIPCION = Strings.Trim(txtINC_DESCRIPCION.Text)
            Simple.INC_TIPO = DevolverTiposCampos("INC_TIPO", cboINC_TIPO.Text, Simple)
            Simple.USU_ID = SessionService.UserId
            Simple.INC_FEC_GRAB = Now
            Simple.INC_ESTADO = DevolverTiposCampos(chkINC_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    resp.rM = Simple.ColocarErrores(txtINC_ID, Simple.VerificarDatos("INC_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtINC_DESCRIPCION, Simple.VerificarDatos("INC_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(cboINC_TIPO, Simple.VerificarDatos("INC_TIPO"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("INC_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkINC_ESTADO, Simple.VerificarDatos("INC_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.Incidencias
        End Sub
        ...
        Private Sub AdicionarElementoCombosEdicion()
            BuscarFormatos(EcboINC_TIPO, Simple, cboINC_TIPO, 0)
        End Sub
...
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkINC_ESTADO = EchkTemporal
            EchkINC_ESTADO.pNombreCampo = "INC_ESTADO"
            ConfigurarCheck_Refrescar(EchkINC_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkINC_ESTADO"
                    Simple.CampoId = EchkINC_ESTADO.pNombreCampo
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                Case "chkINC_ESTADO"
                    vObjetoChk.pValorDefault = EchkINC_ESTADO.pValorDefault
            End Select
        ...
        Handles chkINC_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkINC_ESTADO"
                    If EchkINC_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkINC_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "TextBox"
        Private Sub ConfigurarText()
            ...
            EtxtINC_ID = EtxtTemporal
            EtxtINC_DESCRIPCION = EtxtTemporal
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            FormatearCampos(txtINC_ID, "INC_ID", EtxtINC_ID)
            FormatearCampos(txtINC_DESCRIPCION, "INC_DESCRIPCION", EtxtINC_DESCRIPCION)
            FormatearCampos(cboINC_TIPO, "INC_TIPO", Nothing)
        End Sub
...
#Region "ComboBox"
        Private Sub ConfigurarComboBox()
            EcboINC_TIPO.pNombreCampo = "INC_TIPO"
            cboINC_TIPO.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList
        End Sub
#End Region
...
#Region "TextBox"
        ...
        Handles txtINC_ID.Validated, _
        txtINC_DESCRIPCION.Validated
            Select Case sender.name.ToString
                Case "txtINC_ID"
                    ValidarDatos(EtxtINC_ID, txtINC_ID)
                Case "txtINC_DESCRIPCION"
                    ValidarDatos(EtxtINC_DESCRIPCION, txtINC_DESCRIPCION)
            End Select
        End Sub
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
        Handles txtINC_ID.KeyPress, _
        txtINC_DESCRIPCION.KeyPress
            Select Case sender.name.ToString
                Case "txtINC_ID"
                    oKeyPress(EtxtINC_ID, e)
                Case "txtINC_DESCRIPCION"
                    oKeyPress(EtxtINC_DESCRIPCION, e)
            End Select
        End Sub
