...
Namespace Ladisac.ActivosFijos.Views
    Public Class frmDepartamentosAdministrativos
#Region "Primaria"
        <Dependency()>
...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCDepartamentosAdministrativos

        ' Controlar la clave de la tabla

        ' CheckBox
        Private EchkOFI_ESTADO As New chk
        Private EchkDEA_ESTADO As New chk
        ...
        ' TextBox
        '' PK
        Private EtxtDEA_ID As New txt
        Private EtxtOFI_ID As New txt
        '' Texto
        Private EtxtDEA_DESCRIPCION As New txt

        '' Número

        Private Simple As New Ladisac.BE.DepartamentosAdministrativos
        Private Simple1 As New Ladisac.BE.Oficinas
        Private ErrorData As New Ladisac.BE.DepartamentosAdministrativos.ErrorData
        Private cMisProcedimientos As New Ladisac.MisProcedimientos


        Private Sub LimpiarDatos()
            InicializarValores(txtDEA_ID, ErrorProvider1)
            InicializarValores(txtDEA_DESCRIPCION, ErrorProvider1)
            InicializarValores(txtOFI_ID, ErrorProvider1)
            InicializarValores(txtOFI_DESCRIPCION, ErrorProvider1)
            InicializarValores(chkOFI_ESTADO, ErrorProvider1, False, False, EchkOFI_ESTADO.pValorDefault)
            ColocarValoresDefault(chkOFI_ESTADO)
            InicializarValores(chkDEA_ESTADO, ErrorProvider1, False, False, EchkDEA_ESTADO.pValorDefault)
            ColocarValoresDefault(chkDEA_ESTADO)
        End Sub
        Private Sub HabilitarNuevo()
            txtDEA_ID.Enabled = True
        End Sub
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtDEA_ID)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtDEA_ID.ReadOnly = False
        End Sub
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "PrepararEliminar"
                    ...
                    Simple.DEA_ID = CodigoId
                Case "Load"
                    Simple.Vista = "PrimerAnterior"
                    Simple.DEA_ID = CodigoId
                Case "RegistroNoEncontrado"
                    Simple.DEA_ID = txtDEA_ID.Text.Trim
                Case "NavegarFormulario"
                    CodigoId = CodigoId
                Case "EliminarRegistro"
                    Simple.DEA_ID = txtDEA_ID.Text.Trim
                    CodigoId = txtDEA_ID.Text.Trim
        ...
        Private Sub DatosCabecera()
            Simple.DEA_ID = Strings.Trim(txtDEA_ID.Text)
            Simple.DEA_DESCRIPCION = Strings.Trim(txtDEA_DESCRIPCION.Text)
            Simple.OFI_ID = Strings.Trim(txtOFI_ID.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.DEA_FEC_GRAB = Now
            Simple.DEA_ESTADO = DevolverTiposCampos(chkDEA_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    resp.rM = Simple.ColocarErrores(txtDEA_ID, Simple.VerificarDatos("DEA_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtDEA_DESCRIPCION, Simple.VerificarDatos("DEA_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtOFI_ID, Simple.VerificarDatos("OFI_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("DEA_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkDEA_ESTADO, Simple.VerificarDatos("DEA_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.DepartamentosAdministrativos
        End Sub
        Public Sub FiltrarCampos(ByVal vComportamiento As Integer)
        End Sub
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                If Me.ActiveControl.Name.ToString = "txtOFI_ID" Then EtxtOFI_ID.pTexto2 = Me.ActiveControl.Text
        ...
        Private Sub NombresFormulariosControles()
            EtxtOFI_ID.pOOrm = Simple1
            EtxtOFI_ID.pComportamiento = 1
            EtxtOFI_ID.pOrdenBusqueda = 0
        End Sub
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkOFI_ESTADO = EchkTemporal
            EchkOFI_ESTADO.pNombreCampo = "OFI_ESTADO"

            EchkDEA_ESTADO = EchkTemporal
            EchkDEA_ESTADO.pNombreCampo = "DEA_ESTADO"
            EchkDEA_ESTADO.pValorDefault = CheckState.Checked

            ConfigurarCheck_Refrescar(EchkOFI_ESTADO)
            ConfigurarCheck_Refrescar(EchkDEA_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkOFI_ESTADO"
                    Simple.CampoId = EchkOFI_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkDEA_ESTADO"
                    Simple.CampoId = EchkDEA_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                Case "chkOFI_ESTADO"
                    vObjetoChk.pValorDefault = EchkOFI_ESTADO.pValorDefault
                Case "chkDEA_ESTADO"
                    vObjetoChk.pValorDefault = EchkDEA_ESTADO.pValorDefault
            End Select
        ...
        Handles chkDEA_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkOFI_ESTADO"
                    If EchkOFI_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkOFI_ESTADO)
                    End If
                Case "chkDEA_ESTADO"
                    If EchkDEA_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkDEA_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "TextBox"
        Private Sub ConfigurarText()
            ...
            EtxtDEA_ID = EtxtTemporal
            EtxtDEA_DESCRIPCION = EtxtTemporal
            EtxtOFI_ID = EtxtTemporal

            EtxtOFI_ID.pBusqueda = True
            EtxtOFI_ID.pFormularioConsulta = True

        End Sub
        Private Sub txt_OnKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) _
        Handles txtOFI_ID.KeyDown
            Select Case e.KeyCode
                Case Keys.F1
                    Select Case sender.name.ToString
                        Case "txtOFI_ID"
                            TeclaF1(EtxtOFI_ID, txtOFI_ID)
                    End Select
            End Select
        End Sub
#End Region
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            FormatearCampos(txtDEA_ID, "DEA_ID", EtxtDEA_ID)
            FormatearCampos(txtDEA_DESCRIPCION, "DEA_DESCRIPCION", EtxtDEA_DESCRIPCION)
            FormatearCampos(txtOFI_ID, "OFI_ID", EtxtOFI_ID)
            FormatearCampos(txtOFI_DESCRIPCION, "OFI_DESCRIPCION", Nothing)
        End Sub
        ...
        Private Sub FiltrarOrm()
            EtxtOFI_ID.pOOrm.CadenaFiltrado = ""
        End Sub
#Region "ComboBox"
...
#Region "TextBox"
        ...
        Handles txtOFI_ID.GotFocus
            Select Case sender.name.ToString
                Case "txtOFI_ID"
                    EtxtOFI_ID.pTexto1 = sender.text
            End Select
        End Sub
        Private Sub txt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
        Handles txtOFI_ID.LostFocus
            Select Case sender.name.ToString
                Case "txtOFI_ID"
                    EtxtOFI_ID.pTexto2 = sender.text
            End Select
        End Sub
        Private Sub txt_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtDEA_ID.Validated, _
                txtDEA_DESCRIPCION.Validated, _
                txtOFI_ID.Validated
            Select Case sender.name.ToString
                Case "txtDEA_ID"
                    ValidarDatos(EtxtDEA_ID, txtDEA_ID)
                Case "txtDEA_DESCRIPCION"
                    ValidarDatos(EtxtDEA_DESCRIPCION, txtDEA_DESCRIPCION)
                Case "txtOFI_ID"
                    ValidarDatos(EtxtOFI_ID, txtOFI_ID)
            End Select
        End Sub
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
        Handles txtDEA_ID.KeyPress, _
                txtDEA_DESCRIPCION.KeyPress, _
                txtOFI_ID.KeyPress
            Select Case sender.name.ToString
                Case "txtDEA_ID"
                    oKeyPress(EtxtDEA_ID, e)
                Case "txtDEA_DESCRIPCION"
                    oKeyPress(EtxtDEA_DESCRIPCION, e)
                Case "txtOFI_ID"
                    oKeyPress(EtxtOFI_ID, e)
            End Select
        End Sub
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtOFI_ID.DoubleClick

            Select Case sender.name.ToString
                Case "txtOFI_ID"
                    oDoubleClick(EtxtOFI_ID, txtOFI_ID, "frmOficinas")
            End Select
        End Sub
        Private Sub oDoubleClick(ByVal EtxtTemporal As txt, ByRef txt As TextBox, ByVal e As System.String)
            ...
            If EtxtTemporal.pFormularioConsulta Then
                Select Case e
                    Case "frmOficinas"
                        frmconsulta = Me.ContainerService.Resolve(Of Ladisac.ActivosFijos.Views.frmOficinas)()
                    Case Else
                        Exit Sub
                End Select
