...
Namespace Ladisac.ActivosFijos.Views
    Public Class frmCuentasActivos
#Region "Primaria"
        <Dependency()>
...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCCuentasActivos

        ' Controlar la clave de la tabla

        ' CheckBox
        Private EchkCUC_ESTADO_ACT As New chk
        Private EchkCUC_ESTADO_DEP As New chk
        Private EchkCUC_ESTADO_PRO As New chk
        Private EchkCUC_ESTADO_ACT_ACU As New chk
        Private EchkCUC_ESTADO_DEP_ACU As New chk
        Private EchkCUC_ESTADO_RES As New chk
        Private EchkCUA_ESTADO As New chk
        ...
        ' TextBox
        '' PK
        Private EtxtCUA_ID As New txt
        Private EtxtCUC_ID_ACT As New txt
        Private EtxtCUC_ID_DEP As New txt
        Private EtxtCUC_ID_PRO As New txt
        Private EtxtCUC_ID_ACT_ACU As New txt
        Private EtxtCUC_ID_DEP_ACU As New txt
        Private EtxtCUC_ID_RES As New txt
        '' Texto

        '' Número
        Private EtxtCUA_TASA_ANUAL As New txt
        Private EtxtCUA_MESES As New txt

        Private Simple As New Ladisac.BE.CuentasActivos
        Private Simple1 As New Ladisac.BE.CuentasContables
        Private ErrorData As New Ladisac.BE.CuentasActivos.ErrorData
        Private cMisProcedimientos As New Ladisac.MisProcedimientos


        Private Sub LimpiarDatos()
            InicializarValores(txtCUA_ID, ErrorProvider1)

            InicializarValores(txtCUC_ID_ACT, ErrorProvider1)
            InicializarValores(txtCUC_DESCRIPCION_ACT, ErrorProvider1)
            InicializarValores(chkCUC_ESTADO_ACT, ErrorProvider1, False, False, EchkCUC_ESTADO_ACT.pValorDefault)

            InicializarValores(txtCUC_ID_DEP, ErrorProvider1)
            InicializarValores(txtCUC_DESCRIPCION_DEP, ErrorProvider1)
            InicializarValores(chkCUC_ESTADO_DEP, ErrorProvider1, False, False, EchkCUC_ESTADO_DEP.pValorDefault)

            InicializarValores(txtCUC_ID_PRO, ErrorProvider1)
            InicializarValores(txtCUC_DESCRIPCION_PRO, ErrorProvider1)
            InicializarValores(chkCUC_ESTADO_PRO, ErrorProvider1, False, False, EchkCUC_ESTADO_PRO.pValorDefault)

            InicializarValores(txtCUC_ID_ACT_ACU, ErrorProvider1)
            InicializarValores(txtCUC_DESCRIPCION_ACT_ACU, ErrorProvider1)
            InicializarValores(chkCUC_ESTADO_ACT_ACU, ErrorProvider1, False, False, EchkCUC_ESTADO_ACT_ACU.pValorDefault)

            InicializarValores(txtCUC_ID_DEP_ACU, ErrorProvider1)
            InicializarValores(txtCUC_DESCRIPCION_DEP_ACU, ErrorProvider1)
            InicializarValores(chkCUC_ESTADO_DEP_ACU, ErrorProvider1, False, False, EchkCUC_ESTADO_DEP_ACU.pValorDefault)

            InicializarValores(txtCUC_ID_RES, ErrorProvider1)
            InicializarValores(txtCUC_DESCRIPCION_RES, ErrorProvider1)
            InicializarValores(chkCUC_ESTADO_RES, ErrorProvider1, False, False, EchkCUC_ESTADO_RES.pValorDefault)

            InicializarValores(txtCUA_TASA_ANUAL, ErrorProvider1, EtxtCUA_TASA_ANUAL.pSoloNumeros, EtxtCUA_TASA_ANUAL.pSoloNumerosDecimales)
            InicializarValores(txtCUA_MESES, ErrorProvider1, EtxtCUA_MESES.pSoloNumeros, EtxtCUA_MESES.pSoloNumerosDecimales)
            InicializarValores(chkCUA_ESTADO, ErrorProvider1, False, False, EchkCUA_ESTADO.pValorDefault)

            ColocarValoresDefault(chkCUC_ESTADO_ACT)
            ColocarValoresDefault(chkCUC_ESTADO_DEP)
            ColocarValoresDefault(chkCUC_ESTADO_PRO)
            ColocarValoresDefault(chkCUC_ESTADO_ACT_ACU)
            ColocarValoresDefault(chkCUC_ESTADO_DEP_ACU)
            ColocarValoresDefault(chkCUC_ESTADO_RES)
            ColocarValoresDefault(chkCUA_ESTADO)
        End Sub
        Private Sub HabilitarNuevo()
            txtCUA_ID.Enabled = True
        End Sub
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtCUA_ID)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtCUA_ID.ReadOnly = False
        End Sub
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "PrepararEliminar"
                    ...
                    Simple.CUA_ID = CodigoId
                Case "Load"
                    Simple.Vista = "PrimerAnterior"
                    Simple.CUA_ID = CodigoId
                Case "RegistroNoEncontrado"
                    Simple.CUA_ID = txtCUA_ID.Text.Trim
                Case "NavegarFormulario"
                    CodigoId = CodigoId
                Case "EliminarRegistro"
                    Simple.CUA_ID = txtCUA_ID.Text.Trim
                    CodigoId = txtCUA_ID.Text.Trim
        ...
        Private Sub DatosCabecera()
            Simple.CUA_ID = Strings.Trim(txtCUA_ID.Text)
            Simple.CUC_ID_ACT = Strings.Trim(txtCUC_ID_ACT.Text)
            If Strings.Trim(txtCUC_ID_DEP.Text) = "" Then
                Simple.CUC_ID_DEP = Nothing
            Else
                Simple.CUC_ID_DEP = Strings.Trim(txtCUC_ID_DEP.Text)
            End If
            If Strings.Trim(txtCUC_ID_PRO.Text) = "" Then
                Simple.CUC_ID_PRO = Nothing
            Else
                Simple.CUC_ID_PRO = Strings.Trim(txtCUC_ID_PRO.Text)
            End If
            If Strings.Trim(txtCUC_ID_ACT_ACU.Text) = "" Then
                Simple.CUC_ID_ACT_ACU = Nothing
            Else
                Simple.CUC_ID_ACT_ACU = Strings.Trim(txtCUC_ID_ACT_ACU.Text)
            End If
            If Strings.Trim(txtCUC_ID_DEP_ACU.Text) = "" Then
                Simple.CUC_ID_DEP_ACU = Nothing
            Else
                Simple.CUC_ID_DEP_ACU = Strings.Trim(txtCUC_ID_DEP_ACU.Text)
            End If
            If Strings.Trim(txtCUC_ID_RES.Text) = "" Then
                Simple.CUC_ID_RES = Nothing
            Else
                Simple.CUC_ID_RES = Strings.Trim(txtCUC_ID_RES.Text)
            End If
            Simple.CUA_TASA_ANUAL = CDbl(txtCUA_TASA_ANUAL.Text)
            Simple.CUA_MESES = CDbl(txtCUA_MESES.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.CUA_FEC_GRAB = Now
            Simple.CUA_ESTADO = DevolverTiposCampos(chkCUA_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    resp.rM = Simple.ColocarErrores(txtCUA_ID, Simple.VerificarDatos("CUA_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCUC_ID_ACT, Simple.VerificarDatos("CUC_ID_ACT"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCUC_ID_DEP, Simple.VerificarDatos("CUC_ID_DEP"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCUC_ID_PRO, Simple.VerificarDatos("CUC_ID_PRO"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCUC_ID_ACT_ACU, Simple.VerificarDatos("CUC_ID_ACT_ACU"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCUC_ID_DEP_ACU, Simple.VerificarDatos("CUC_ID_DEP_ACU"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCUC_ID_RES, Simple.VerificarDatos("CUC_ID_RES"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCUA_TASA_ANUAL, Simple.VerificarDatos("CUA_TASA_ANUAL"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCUA_MESES, Simple.VerificarDatos("CUA_MESES"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("CUA_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkCUA_ESTADO, Simple.VerificarDatos("CUA_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.CuentasActivos
        End Sub
        Public Sub FiltrarCampos(ByVal vComportamiento As Integer)
        End Sub
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                If Me.ActiveControl.Name.ToString = "txtCUC_ID_ACT" Then EtxtCUC_ID_ACT.pTexto2 = Me.ActiveControl.Text
                If Me.ActiveControl.Name.ToString = "txtCUC_ID_DEP" Then EtxtCUC_ID_DEP.pTexto2 = Me.ActiveControl.Text
                If Me.ActiveControl.Name.ToString = "txtCUC_ID_PRO" Then EtxtCUC_ID_PRO.pTexto2 = Me.ActiveControl.Text
                If Me.ActiveControl.Name.ToString = "txtCUC_ID_ACT_ACU" Then EtxtCUC_ID_ACT_ACU.pTexto2 = Me.ActiveControl.Text
                If Me.ActiveControl.Name.ToString = "txtCUC_ID_DEP_ACU" Then EtxtCUC_ID_DEP_ACU.pTexto2 = Me.ActiveControl.Text
                If Me.ActiveControl.Name.ToString = "txtCUC_ID_RES" Then EtxtCUC_ID_RES.pTexto2 = Me.ActiveControl.Text
        ...
        Private Sub NombresFormulariosControles()
            EtxtCUC_ID_ACT.pOOrm = Simple1
            EtxtCUC_ID_ACT.pComportamiento = 1
            EtxtCUC_ID_ACT.pOrdenBusqueda = 0

            EtxtCUC_ID_DEP.pOOrm = Simple1
            EtxtCUC_ID_DEP.pComportamiento = 2
            EtxtCUC_ID_DEP.pOrdenBusqueda = 0

            EtxtCUC_ID_PRO.pOOrm = Simple1
            EtxtCUC_ID_PRO.pComportamiento = 3
            EtxtCUC_ID_PRO.pOrdenBusqueda = 0

            EtxtCUC_ID_ACT_ACU.pOOrm = Simple1
            EtxtCUC_ID_ACT_ACU.pComportamiento = 4
            EtxtCUC_ID_ACT_ACU.pOrdenBusqueda = 0

            EtxtCUC_ID_DEP_ACU.pOOrm = Simple1
            EtxtCUC_ID_DEP_ACU.pComportamiento = 5
            EtxtCUC_ID_DEP_ACU.pOrdenBusqueda = 0

            EtxtCUC_ID_RES.pOOrm = Simple1
            EtxtCUC_ID_RES.pComportamiento = 6
            EtxtCUC_ID_RES.pOrdenBusqueda = 0
        End Sub
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkCUC_ESTADO_ACT = EchkTemporal
            EchkCUC_ESTADO_DEP = EchkTemporal
            EchkCUC_ESTADO_PRO = EchkTemporal
            EchkCUC_ESTADO_ACT_ACU = EchkTemporal
            EchkCUC_ESTADO_DEP_ACU = EchkTemporal
            EchkCUC_ESTADO_RES = EchkTemporal
            EchkCUA_ESTADO = EchkTemporal

            EchkCUC_ESTADO_ACT.pNombreCampo = "CUC_ESTADO_ACT"
            EchkCUC_ESTADO_DEP.pNombreCampo = "CUC_ESTADO_DEP"
            EchkCUC_ESTADO_PRO.pNombreCampo = "CUC_ESTADO_PRO"
            EchkCUC_ESTADO_ACT_ACU.pNombreCampo = "CUC_ESTADO_ACT_ACU"
            EchkCUC_ESTADO_DEP_ACU.pNombreCampo = "CUC_ESTADO_DEP_ACU"
            EchkCUC_ESTADO_RES.pNombreCampo = "CUC_ESTADO_RES"
            EchkCUA_ESTADO.pNombreCampo = "CUA_ESTADO"

            EchkCUA_ESTADO.pValorDefault = CheckState.Checked

            ConfigurarCheck_Refrescar(EchkCUC_ESTADO_ACT)
            ConfigurarCheck_Refrescar(EchkCUC_ESTADO_DEP)
            ConfigurarCheck_Refrescar(EchkCUC_ESTADO_PRO)
            ConfigurarCheck_Refrescar(EchkCUC_ESTADO_ACT_ACU)
            ConfigurarCheck_Refrescar(EchkCUC_ESTADO_DEP_ACU)
            ConfigurarCheck_Refrescar(EchkCUC_ESTADO_RES)
            ConfigurarCheck_Refrescar(EchkCUA_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkCUC_ESTADO_ACT"
                    Simple.CampoId = EchkCUC_ESTADO_ACT.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkCUC_ESTADO_DEP"
                    Simple.CampoId = EchkCUC_ESTADO_DEP.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkCUC_ESTADO_PRO"
                    Simple.CampoId = EchkCUC_ESTADO_PRO.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkCUC_ESTADO_ACT_ACU"
                    Simple.CampoId = EchkCUC_ESTADO_ACT_ACU.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkCUC_ESTADO_DEP_ACU"
                    Simple.CampoId = EchkCUC_ESTADO_DEP_ACU.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkCUC_ESTADO_RES"
                    Simple.CampoId = EchkCUC_ESTADO_RES.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkCUA_ESTADO"
                    Simple.CampoId = EchkCUA_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                Case "chkCUC_ESTADO_ACT"
                    vObjetoChk.pValorDefault = EchkCUC_ESTADO_ACT.pValorDefault
                Case "chkCUC_ESTADO_DEP"
                    vObjetoChk.pValorDefault = EchkCUC_ESTADO_DEP.pValorDefault
                Case "chkCUC_ESTADO_PRO"
                    vObjetoChk.pValorDefault = EchkCUC_ESTADO_PRO.pValorDefault
                Case "chkCUC_ESTADO_ACT_ACU"
                    vObjetoChk.pValorDefault = EchkCUC_ESTADO_ACT_ACU.pValorDefault
                Case "chkCUC_ESTADO_DEP_ACU"
                    vObjetoChk.pValorDefault = EchkCUC_ESTADO_DEP_ACU.pValorDefault
                Case "chkCUC_ESTADO_RES"
                    vObjetoChk.pValorDefault = EchkCUC_ESTADO_RES.pValorDefault
                Case "chkCUA_ESTADO"
                    vObjetoChk.pValorDefault = EchkCUA_ESTADO.pValorDefault
            End Select
        ...
        Handles chkCUA_ESTADO.CheckedChanged, _
                chkCUC_ESTADO_ACT.CheckedChanged, _
                chkCUC_ESTADO_DEP.CheckedChanged, _
                chkCUC_ESTADO_PRO.CheckedChanged, _
                chkCUC_ESTADO_ACT_ACU.CheckedChanged, _
                chkCUC_ESTADO_DEP_ACU.CheckedChanged, _
                chkCUC_ESTADO_RES.CheckedChanged
            Select Case sender.name.ToString
                Case "chkCUC_ESTADO_ACT"
                    If EchkCUC_ESTADO_ACT.pFormatearTexto Then
                        InicializarTextoCheck(EchkCUC_ESTADO_ACT)
                    End If
                Case "chkCUC_ESTADO_DEP"
                    If EchkCUC_ESTADO_DEP.pFormatearTexto Then
                        InicializarTextoCheck(EchkCUC_ESTADO_DEP)
                    End If
                Case "chkCUC_ESTADO_PRO"
                    If EchkCUC_ESTADO_PRO.pFormatearTexto Then
                        InicializarTextoCheck(EchkCUC_ESTADO_PRO)
                    End If
                Case "chkCUC_ESTADO_ACT_ACU"
                    If EchkCUC_ESTADO_ACT_ACU.pFormatearTexto Then
                        InicializarTextoCheck(EchkCUC_ESTADO_ACT_ACU)
                    End If
                Case "chkCUC_ESTADO_DEP_ACU"
                    If EchkCUC_ESTADO_DEP_ACU.pFormatearTexto Then
                        InicializarTextoCheck(EchkCUC_ESTADO_DEP_ACU)
                    End If
                Case "chkCUC_ESTADO_RES"
                    If EchkCUC_ESTADO_RES.pFormatearTexto Then
                        InicializarTextoCheck(EchkCUC_ESTADO_RES)
                    End If
                Case "chkCUA_ESTADO"
                    If EchkCUA_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkCUA_ESTADO)
                    End If
            End Select
        End Sub
        Private Sub InicializarTextoCheck(ByVal vObjeto As chk)
            Select Case vObjeto.pNombreCampo
                Case "CUC_ESTADO_ACT"
                    ...
                    End With
                Case "CUC_ESTADO_DEP"
                    With chkCUC_ESTADO_DEP
                        If .CheckState = CheckState.Checked Then .Text = vObjeto.vEstado1
                        If .CheckState = CheckState.Unchecked Then .Text = vObjeto.vEstado0
                        If .CheckState = CheckState.Indeterminate Then .Text = vObjeto.vEstadoX
                    End With
                Case "CUC_ESTADO_PRO"
                    With chkCUC_ESTADO_PRO
                        If .CheckState = CheckState.Checked Then .Text = vObjeto.vEstado1
                        If .CheckState = CheckState.Unchecked Then .Text = vObjeto.vEstado0
                        If .CheckState = CheckState.Indeterminate Then .Text = vObjeto.vEstadoX
                    End With
                Case "CUC_ESTADO_ACT_ACU"
                    With chkCUC_ESTADO_ACT_ACU
                        If .CheckState = CheckState.Checked Then .Text = vObjeto.vEstado1
                        If .CheckState = CheckState.Unchecked Then .Text = vObjeto.vEstado0
                        If .CheckState = CheckState.Indeterminate Then .Text = vObjeto.vEstadoX
                    End With
                Case "CUC_ESTADO_DEP_ACU"
                    With chkCUC_ESTADO_DEP_ACU
                        If .CheckState = CheckState.Checked Then .Text = vObjeto.vEstado1
                        If .CheckState = CheckState.Unchecked Then .Text = vObjeto.vEstado0
                        If .CheckState = CheckState.Indeterminate Then .Text = vObjeto.vEstadoX
                    End With
                Case "CUC_ESTADO_RES"
                    With chkCUC_ESTADO_RES
                        If .CheckState = CheckState.Checked Then .Text = vObjeto.vEstado1
                        If .CheckState = CheckState.Unchecked Then .Text = vObjeto.vEstado0
                        If .CheckState = CheckState.Indeterminate Then .Text = vObjeto.vEstadoX
                    End With
...
#Region "TextBox"
        Private Sub ConfigurarText()
            ...
            EtxtCUA_ID = EtxtTemporal
            EtxtCUC_ID_ACT = EtxtTemporal
            EtxtCUC_ID_DEP = EtxtTemporal
            EtxtCUC_ID_PRO = EtxtTemporal
            EtxtCUC_ID_ACT_ACU = EtxtTemporal
            EtxtCUC_ID_DEP_ACU = EtxtTemporal
            EtxtCUC_ID_RES = EtxtTemporal
            EtxtCUA_TASA_ANUAL = EtxtTemporal
            EtxtCUA_MESES = EtxtTemporal

            EtxtCUC_ID_ACT.pBusqueda = True
            EtxtCUC_ID_ACT.pFormularioConsulta = True

            EtxtCUC_ID_DEP.pBusqueda = True
            EtxtCUC_ID_DEP.pFormularioConsulta = True

            EtxtCUC_ID_PRO.pBusqueda = True
            EtxtCUC_ID_PRO.pFormularioConsulta = True

            EtxtCUC_ID_ACT_ACU.pBusqueda = True
            EtxtCUC_ID_ACT_ACU.pFormularioConsulta = True

            EtxtCUC_ID_DEP_ACU.pBusqueda = True
            EtxtCUC_ID_DEP_ACU.pFormularioConsulta = True

            EtxtCUC_ID_RES.pBusqueda = True
            EtxtCUC_ID_RES.pFormularioConsulta = True

        End Sub
        Private Sub txt_OnKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) _
        Handles txtCUC_ID_ACT.KeyDown, _
                txtCUC_ID_DEP.KeyDown, _
                txtCUC_ID_PRO.KeyDown, _
                txtCUC_ID_ACT_ACU.KeyDown, _
                txtCUC_ID_DEP_ACU.KeyDown, _
                txtCUC_ID_RES.KeyDown
            Select Case e.KeyCode
                Case Keys.F1
                    Select Case sender.name.ToString
                        Case "txtCUC_ID_ACT"
                            TeclaF1(EtxtCUC_ID_ACT, txtCUC_ID_ACT)
                        Case "txtCUC_ID_DEP"
                            TeclaF1(EtxtCUC_ID_DEP, txtCUC_ID_DEP)
                        Case "txtCUC_ID_PRO"
                            TeclaF1(EtxtCUC_ID_PRO, txtCUC_ID_PRO)
                        Case "txtCUC_ID_ACT_ACU"
                            TeclaF1(EtxtCUC_ID_ACT_ACU, txtCUC_ID_ACT_ACU)
                        Case "txtCUC_ID_DEP_ACU"
                            TeclaF1(EtxtCUC_ID_DEP_ACU, txtCUC_ID_DEP_ACU)
                        Case "txtCUC_ID_RES"
                            TeclaF1(EtxtCUC_ID_RES, txtCUC_ID_RES)
                    End Select
            End Select
        End Sub
#End Region
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            FormatearCampos(txtCUA_ID, "CUA_ID", EtxtCUA_ID)
            FormatearCampos(txtCUC_ID_ACT, "CUC_ID_ACT", EtxtCUC_ID_ACT)
            FormatearCampos(txtCUC_ID_DEP, "CUC_ID_DEP", EtxtCUC_ID_DEP)
            FormatearCampos(txtCUC_ID_PRO, "CUC_ID_PRO", EtxtCUC_ID_PRO)
            FormatearCampos(txtCUC_ID_ACT_ACU, "CUC_ID_ACT_ACU", EtxtCUC_ID_ACT_ACU)
            FormatearCampos(txtCUC_ID_DEP_ACU, "CUC_ID_DEP_ACU", EtxtCUC_ID_DEP_ACU)
            FormatearCampos(txtCUC_ID_RES, "CUC_ID_RES", EtxtCUC_ID_RES)
            FormatearCampos(txtCUA_TASA_ANUAL, "CUA_TASA_ANUAL", EtxtCUA_TASA_ANUAL)
            FormatearCampos(txtCUA_MESES, "CUA_MESES", EtxtCUA_MESES)
        End Sub
        ...
        Private Sub FiltrarOrm()
            EtxtCUC_ID_ACT.pOOrm.CadenaFiltrado = ""
            EtxtCUC_ID_DEP.pOOrm.CadenaFiltrado = ""
            EtxtCUC_ID_PRO.pOOrm.CadenaFiltrado = ""
            EtxtCUC_ID_ACT_ACU.pOOrm.CadenaFiltrado = ""
            EtxtCUC_ID_DEP_ACU.pOOrm.CadenaFiltrado = ""
            EtxtCUC_ID_RES.pOOrm.CadenaFiltrado = ""
        End Sub
#Region "ComboBox"
...
#Region "TextBox"
        ...
        Handles txtCUC_ID_ACT.GotFocus, _
                txtCUC_ID_DEP.GotFocus, _
                txtCUC_ID_PRO.GotFocus, _
                txtCUC_ID_ACT_ACU.GotFocus, _
                txtCUC_ID_DEP_ACU.GotFocus, _
                txtCUC_ID_RES.GotFocus

            Select Case sender.name.ToString
                Case "txtCUC_ID_ACT"
                    EtxtCUC_ID_ACT.pTexto1 = sender.text
                Case "txtCUC_ID_DEP"
                    EtxtCUC_ID_DEP.pTexto1 = sender.text
                Case "txtCUC_ID_PRO"
                    EtxtCUC_ID_PRO.pTexto1 = sender.text
                Case "txtCUC_ID_ACT_ACU"
                    EtxtCUC_ID_ACT_ACU.pTexto1 = sender.text
                Case "txtCUC_ID_DEP_ACU"
                    EtxtCUC_ID_DEP_ACU.pTexto1 = sender.text
                Case "txtCUC_ID_RES"
                    EtxtCUC_ID_RES.pTexto1 = sender.text
            End Select
        End Sub
        Private Sub txt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
        Handles txtCUC_ID_ACT.LostFocus, _
                txtCUC_ID_DEP.LostFocus, _
                txtCUC_ID_PRO.LostFocus, _
                txtCUC_ID_ACT_ACU.LostFocus, _
                txtCUC_ID_DEP_ACU.LostFocus, _
                txtCUC_ID_RES.LostFocus

            Select Case sender.name.ToString
                Case "txtCUC_ID_ACT"
                    EtxtCUC_ID_ACT.pTexto2 = sender.text
                Case "txtCUC_ID_DEP"
                    EtxtCUC_ID_DEP.pTexto2 = sender.text
                Case "txtCUC_ID_PRO"
                    EtxtCUC_ID_PRO.pTexto2 = sender.text
                Case "txtCUC_ID_ACT_ACU"
                    EtxtCUC_ID_ACT_ACU.pTexto2 = sender.text
                Case "txtCUC_ID_DEP_ACU"
                    EtxtCUC_ID_DEP_ACU.pTexto2 = sender.text
                Case "txtCUC_ID_RES"
                    EtxtCUC_ID_RES.pTexto2 = sender.text
            End Select
        End Sub
        Private Sub txt_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtCUA_ID.Validated, _
                txtCUC_ID_ACT.Validated, _
                txtCUC_ID_DEP.Validated, _
                txtCUC_ID_PRO.Validated, _
                txtCUC_ID_ACT_ACU.Validated, _
                txtCUC_ID_DEP_ACU.Validated, _
                txtCUC_ID_RES.Validated, _
                txtCUA_TASA_ANUAL.Validated, _
                txtCUA_MESES.Validated
            Select Case sender.name.ToString
                Case "txtCUA_ID"
                    ValidarDatos(EtxtCUA_ID, txtCUA_ID)
                Case "txtCUC_ID_ACT"
                    ValidarDatos(EtxtCUC_ID_ACT, txtCUC_ID_ACT)
                Case "txtCUC_ID_DEP"
                    ValidarDatos(EtxtCUC_ID_DEP, txtCUC_ID_DEP)
                Case "txtCUC_ID_PRO"
                    ValidarDatos(EtxtCUC_ID_PRO, txtCUC_ID_PRO)
                Case "txtCUC_ID_ACT_ACU"
                    ValidarDatos(EtxtCUC_ID_ACT_ACU, txtCUC_ID_ACT_ACU)
                Case "txtCUC_ID_DEP_ACU"
                    ValidarDatos(EtxtCUC_ID_DEP_ACU, txtCUC_ID_DEP_ACU)
                Case "txtCUC_ID_RES"
                    ValidarDatos(EtxtCUC_ID_RES, txtCUC_ID_RES)
                Case "txtCUA_TASA_ANUAL"
                    ValidarDatos(EtxtCUA_TASA_ANUAL, txtCUA_TASA_ANUAL)
                Case "txtCUA_MESES"
                    ValidarDatos(EtxtCUA_MESES, txtCUA_MESES)
            End Select
        End Sub
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
        Handles txtCUA_ID.KeyPress, _
        txtCUC_ID_ACT.KeyPress, _
        txtCUC_ID_DEP.KeyPress, _
        txtCUC_ID_PRO.KeyPress, _
        txtCUC_ID_ACT_ACU.KeyPress, _
        txtCUC_ID_DEP_ACU.KeyPress, _
        txtCUC_ID_RES.KeyPress, _
        txtCUA_TASA_ANUAL.KeyPress, _
        txtCUA_MESES.KeyPress
            Select Case sender.name.ToString
                Case "txtCUA_ID"
                    oKeyPress(EtxtCUA_ID, e)
                Case "txtCUC_ID_ACT"
                    oKeyPress(EtxtCUC_ID_ACT, e)
                Case "txtCUC_ID_DEP"
                    oKeyPress(EtxtCUC_ID_DEP, e)
                Case "txtCUC_ID_PRO"
                    oKeyPress(EtxtCUC_ID_PRO, e)
                Case "txtCUC_ID_ACT_ACU"
                    oKeyPress(EtxtCUC_ID_ACT_ACU, e)
                Case "txtCUC_ID_DEP_ACU"
                    oKeyPress(EtxtCUC_ID_DEP_ACU, e)
                Case "txtCUC_ID_RES"
                    oKeyPress(EtxtCUC_ID_RES, e)
                Case "txtCUA_TASA_ANUAL"
                    oKeyPress(EtxtCUA_TASA_ANUAL, e)
                Case "txtCUA_MESES"
                    oKeyPress(EtxtCUA_MESES, e)
            End Select
        End Sub
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtCUC_ID_ACT.DoubleClick, _
                txtCUC_ID_DEP.DoubleClick, _
                txtCUC_ID_PRO.DoubleClick, _
                txtCUC_ID_ACT_ACU.DoubleClick, _
                txtCUC_ID_DEP_ACU.DoubleClick, _
                txtCUC_ID_RES.DoubleClick

            Select Case sender.name.ToString
                Case "txtCUC_ID_ACT"
                    oDoubleClick(EtxtCUC_ID_ACT, txtCUC_ID_ACT, "")
                Case "txtCUC_ID_DEP"
                    oDoubleClick(EtxtCUC_ID_DEP, txtCUC_ID_DEP, "")
                Case "txtCUC_ID_PRO"
                    oDoubleClick(EtxtCUC_ID_PRO, txtCUC_ID_PRO, "")
                Case "txtCUC_ID_ACT_ACU"
                    oDoubleClick(EtxtCUC_ID_ACT_ACU, txtCUC_ID_ACT_ACU, "")
                Case "txtCUC_ID_DEP_ACU"
                    oDoubleClick(EtxtCUC_ID_DEP_ACU, txtCUC_ID_DEP_ACU, "")
                Case "txtCUC_ID_RES"
                    oDoubleClick(EtxtCUC_ID_RES, txtCUC_ID_RES, "")
            End Select
        End Sub
        Private Sub oDoubleClick(ByVal EtxtTemporal As txt, ByRef txt As TextBox, ByVal e As System.String)
            ...
            If EtxtTemporal.pFormularioConsulta Then
                Select Case e
                    Case "frm"
                        'frmconsulta = Me.ContainerService.Resolve(Of Ladisac.Facturacion.Views.frm)()
                    Case Else
                        Exit Sub
                End Select
