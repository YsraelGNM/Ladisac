...
Namespace Ladisac.Maestros.Views
    Public Class frmCentroCostos
        ...
        <Dependency()> _
        Public Property IBCCentroCostos As Ladisac.BL.IBCCentroCostos
        ...
        Private EchkCCO_ESTADO As New chk
        Private EtxtCCO_ID As New txt

        Private Simple As New Ladisac.BE.CentroCostos
        Private ErrorData As New Ladisac.BE.CentroCostos.ErrorData
        ...
        Private Sub LimpiarDatos()
            txtCCO_ID.Text = ""
            ErrorProvider1.SetError(txtCCO_ID, Nothing)

            txtCCO_DESCRIPCION.Text = ""
            ErrorProvider1.SetError(txtCCO_DESCRIPCION, Nothing)

            txtCCO_CUC_ID.Text = ""
            ErrorProvider1.SetError(txtCCO_CUC_ID, Nothing)

            txtCCO_NIVEL.Text = ""
            ErrorProvider1.SetError(txtCCO_NIVEL, Nothing)

            chkCCO_ESTADO.Checked = Nothing
            chkCCO_ESTADO.CheckState = EchkCCO_ESTADO.pValorDefault
            ErrorProvider1.SetError(chkCCO_ESTADO, Nothing)
        End Sub
        Private Sub HabilitarNuevo()
            txtCCO_ID.Enabled = True
            txtCCO_ID.ReadOnly = False
        End Sub
        Private Sub ValoresDefaultNuevo()
            ColocarValoresDefault(chkCCO_ESTADO)
        End Sub
        Public Sub ColocarValoresDefault(ByVal vObjeto As CheckBox)
            Dim vObjetoChk As New chk
            Select Case vObjeto.Name
                Case "chkCCO_ESTADO"
                    vObjetoChk.pValorDefault = EchkCCO_ESTADO.pValorDefault
            End Select
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtCCO_ID)
        End Sub
        ...
        Private Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "EliminarRegistro"
                    Simple.CCO_ID = txtCCO_ID.Text.Trim
                    CodigoId = txtCCO_ID.Text.Trim
        ...
        Private Function Ingresar() As Boolean
            ...
            End If
            Simple = Nothing
            Simple = New Ladisac.BE.CentroCostos
            Return Ingresar
        End Function
        Private Sub DatosCabecera()
            Dim vCCO_ID As String = ""
            vCCO_ID = Strings.Trim(txtCCO_ID.Text)
            If IsNumeric(vCCO_ID) Then
                Simple.CCO_ID = vCCO_ID
            Else
                Simple.CCO_ID = ""
            End If

            Simple.CCO_DESCRIPCION = Strings.Trim(txtCCO_DESCRIPCION.Text)
            Simple.CCO_CUC_ID = Strings.Trim(txtCCO_CUC_ID.Text)
            If IsNumeric(txtCCO_NIVEL.Text) Then
                Simple.CCO_NIVEL = CDbl(txtCCO_NIVEL.Text)
            Else
                Simple.CCO_NIVEL = 0
            End If

            Simple.USU_ID = SessionService.UserId
            Simple.CCO_FEC_GRAB = Now
            Simple.CCO_ESTADO = DevolverTiposCampos(chkCCO_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkCCO_ESTADO"
                    Simple.CampoId = EchkCCO_ESTADO.pNombreCampo
        ...
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    resp.rM = Simple.ColocarErrores(txtCCO_ID, Simple.VerificarDatos("CCO_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCCO_DESCRIPCION, Simple.VerificarDatos("CCO_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCCO_CUC_ID, Simple.VerificarDatos("CCO_CUC_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCCO_NIVEL, Simple.VerificarDatos("CCO_NIVEL"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(lblTitle, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(lblTitle, Simple.VerificarDatos("CCO_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkCCO_ESTADO, Simple.VerificarDatos("CCO_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Function InsertarDatos() As Boolean
            ...
            Simple.MarkAsAdded()
            vRespuestaLocal = Me.IBCCentroCostos.Mantenimiento(Simple)
        ...
        Private Function ActualizarDatos() As Boolean
            ...
            ActualizarDatos = (Me.IBCCentroCostos.Mantenimiento(Simple) > 0)
        End Function
        ...
        Private Function EliminarRegistro() As Boolean
            ...
            If (Me.IBCCentroCostos.Mantenimiento(Simple) > 0) Then
        ...
        Private Sub frm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            ...
            Try
                ...
                FormatearCampos(txtCCO_ID, "CCO_ID")
                FormatearCampos(txtCCO_DESCRIPCION, "CCO_DESCRIPCION", False)
                FormatearCampos(txtCCO_CUC_ID, "CCO_CUC_ID")
                FormatearCampos(txtCCO_NIVEL, "CCO_NIVEL")
        ...
        Private Sub ConfigurarCheck()
            EchkCCO_ESTADO.pFormatearTexto = True
            EchkCCO_ESTADO.pNombreCampo = "CCO_ESTADO"
            EchkCCO_ESTADO.pSimple = Simple
            EchkCCO_ESTADO.pValorDefault = CheckState.Checked
            EchkCCO_ESTADO.vEstado0 = ""
            EchkCCO_ESTADO.vEstado1 = ""
            EchkCCO_ESTADO.vEstadoX = ""
            ConfigurarCheck_Refrescar(EchkCCO_ESTADO)
        End Sub
        ...
        Public Sub Check_Refrescar()
            InicializarTexto(EchkCCO_ESTADO)
        End Sub
        Private Sub ConfigurarText()
            EtxtCCO_ID.pTexto1 = ""
            EtxtCCO_ID.pTexto2 = ""
            EtxtCCO_ID.pSoloNumerosDecimales = False
            EtxtCCO_ID.pSoloNumeros = False
            EtxtCCO_ID.pParteEntera = 0
            EtxtCCO_ID.pParteDecimal = 0
            EtxtCCO_ID.pMinusculaMayuscula = True
            EtxtCCO_ID.pBusqueda = False
            EtxtCCO_ID.pCadenaFiltrado = ""

            EtxtCCO_ID.pOOrm = Nothing
            EtxtCCO_ID.pFormularioConsulta = Nothing

            EtxtCCO_ID.pComportamiento = Nothing
            EtxtCCO_ID.pOrdenBusqueda = 0
        End Sub
        ...
        End Sub
        Private Sub FormatearCampos(ByRef oObjeto As Object, ByVal NombreCampo As String, Optional ByVal VerificaAncho As Boolean = True)
            FormatearCampos(oObjeto, NombreCampo, Simple.vArrayDatosComboBox, Simple.vElementosDatosComboBox - 1, VerificaAncho)
        End Sub
        Private Sub FormatearCampos(ByRef oObjeto As Object, ByVal NombreCampo As String, ByVal vArrayDatosComboBox As Object, ByVal vElementos As Int16, ByVal vVerificaAncho As Boolean)
            For Fila = 0 To vElementos
                If vArrayDatosComboBox(Fila).NombreCampo.ToString = NombreCampo Then
                    If vArrayDatosComboBox(Fila).Tipo.ToString = "char" Or _
                        ...
                        If vVerificaAncho Then oObjeto.Width = vArrayDatosComboBox(Fila).Ancho
                    End If
        ...
        Private Sub chk_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles chkCCO_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkCCO_ESTADO"
                    If EchkCCO_ESTADO.pFormatearTexto Then
                        InicializarTexto(EchkCCO_ESTADO)
                    End If
            End Select
        End Sub
