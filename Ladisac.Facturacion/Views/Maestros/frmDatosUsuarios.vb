...
#Region "Declaraciones - Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCDatosUsuarios

        <Dependency()> _
        Public Property IBCDetalle As Ladisac.BL.IBCPuntoVentaDatosUsuarios

        ' Controlar la clave de la tabla

        ' CheckBox
        Private EchkPER_ESTADO As New chk
        Private EchkDAU_ESTADO As New chk
        ...
        Private Compuesto As New Ladisac.BE.DatosUsuarios
        Private Compuesto1 As New Ladisac.BE.PuntoVentaDatosUsuarios
        Private ErrorData As New Ladisac.BE.DatosUsuarios.ErrorData
...
#Region "Secundaria"
        Private Sub LimpiarDatos()
            ...
            InicializarValores(txtUSU_ID, ErrorProvider1)
            InicializarValores(txtPER_ID, ErrorProvider1)
            InicializarValores(txtPER_DESCRIPCION, ErrorProvider1)
            InicializarValores(chkPER_ESTADO, ErrorProvider1, False, False, EchkDAU_ESTADO.pValorDefault)
            ColocarValoresDefault(chkPER_ESTADO)
            InicializarValores(chkDAU_ESTADO, ErrorProvider1, False, False, EchkDAU_ESTADO.pValorDefault)
            ColocarValoresDefault(chkDAU_ESTADO)
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtDAU_ID)
        End Sub
        ...
        Private Sub AdicionarFilasGrid()
            dgvDetalle.Rows.Add("", "", "", "AMBOS", "AMBOS", "AMBOS", "ACTIVO", False)
                ...
                Try
                    With dgvDetalle.Rows(dgvDetalle.CurrentRow.Index)
                        If .Cells("cEstadoRegistro").Value Then
                            eRegistrosEliminar(eRegistrosEliminar.Count() - 1).cDAU_ID = txtDAU_ID.Text.ToString
                            eRegistrosEliminar(eRegistrosEliminar.Count() - 1).cPVE_ID = .Cells("cPVE_ID").Value.ToString()
        ...
        Private Function ProcesarDatosDetalle() As Int16
            ...
            While (dgvDetalle.Rows.Count() > vFilGrid)
                With dgvDetalle.Rows(vFilGrid)
                    ...
                    Compuesto1.DAU_ID = txtDAU_ID.Text.Trim
                    Compuesto1.PVE_ID = .Cells("cPVE_ID").Value
                    Compuesto1.PDU_TIPO_LISTA = DevolverTiposCampos("PDU_TIPO_LISTA", .Cells("cPDU_TIPO_LISTA").Value.ToString, Compuesto1)
                    Compuesto1.PDU_ENTREGA_PLANTA = DevolverTiposCampos("PDU_ENTREGA_PLANTA", .Cells("cPDU_ENTREGA_PLANTA").Value.ToString, Compuesto1)
                    Compuesto1.PDU_ENTREGA_PUNTO_VENTA = DevolverTiposCampos("PDU_ENTREGA_PUNTO_VENTA", .Cells("cPDU_ENTREGA_PUNTO_VENTA").Value.ToString, Compuesto1)
                    Compuesto1.USU_ID = SessionService.UserId
                    Compuesto1.PDU_FEC_GRAB = Now
                    Compuesto1.PDU_ESTADO = DevolverTiposCampos("PDU_ESTADO", .Cells("cPDU_ESTADO").Value.ToString, Compuesto1)
        ...
        Private Function EliminarRegistroDetalle() As Int16
            ...
            Else
                ...
                For fila = 1 To eRegistrosEliminar.Count - 2
                    vMensajeErrorOrm = ""
                    InicializarOrmDetalle()
                    EliminarRegistroDetalle = Me.IBCDetalle.DeleteRegistro(Compuesto1, eRegistrosEliminar(fila).cDAU_ID, eRegistrosEliminar(fila).cPVE_ID)
        ...
        Private Sub DatosCabecera()
            ...
            Compuesto.USU_ID_CODIGO = SessionService.UserId
            Compuesto.DAU_FEC_GRAB = Now
            Compuesto.DAU_ESTADO = DevolverTiposCampos(chkDAU_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Compuesto.ColocarErrores(pnCuerpo, Compuesto.VerificarDatos("USU_ID_CODIGO"), ErrorProvider1)
                    resp.rM = Compuesto.ColocarErrores(btnImagen, Compuesto.VerificarDatos("DAU_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Compuesto.ColocarErrores(chkDAU_ESTADO, Compuesto.VerificarDatos("DAU_ESTADO"), ErrorProvider1)
                Case "Detalle"
        ...
        Private Sub InicializarOrm()
            InicializarOrmDetalle()
            Compuesto = Nothing
            Compuesto = New Ladisac.BE.DatosUsuarios
        End Sub
        Private Sub InicializarOrmDetalle()
            Compuesto1 = Nothing
            Compuesto1 = New Ladisac.BE.PuntoVentaDatosUsuarios
        End Sub
        ...
        End Function
        Private Function EliminarDetalle(ByVal oOrm As PuntoVentaDatosUsuarios) As Int16
            Return 1
        End Function
        ...
        Private Sub NombresFormulariosControles()
            EtxtDAU_ID.pOOrm = New Ladisac.BE.DatosUsuarios
            EtxtDAU_ID.pComportamiento = 0
            EtxtDAU_ID.pOrdenBusqueda = 0

            EtxtUSU_ID.pOOrm = New Ladisac.BE.Usuarios
            EtxtUSU_ID.pComportamiento = 2
            EtxtUSU_ID.pOrdenBusqueda = 0

            EtxtPER_ID.pOOrm = New Ladisac.BE.Personas
            EtxtPER_ID.pComportamiento = 3
            EtxtPER_ID.pOrdenBusqueda = 0
            EtxtPER_ID.pOOrm.CadenaFiltrado = " and PER_ID in (SELECT PER_ID FROM vwRolPersonaTipoPersona WHERE PER_TRABAJADOR='SI')"

            EtxtPVE_ID.pOOrm = New Ladisac.BE.PuntoVenta
            EtxtPVE_ID.pComportamiento = 4
...
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkDAU_ESTADO = EchkTemporal
            EchkDAU_ESTADO.pNombreCampo = "DAU_ESTADO"
            EchkDAU_ESTADO.pValorDefault = CheckState.Checked
            ConfigurarCheck_Refrescar(EchkDAU_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkPER_ESTADO"
                    Compuesto.CampoId = EchkPER_ESTADO.pNombreCampo
                    Compuesto.Dato = oObjeto.Text
                Case "chkDAU_ESTADO"
                    Compuesto.CampoId = EchkDAU_ESTADO.pNombreCampo
                    Compuesto.Dato = oObjeto.Text
            End Select
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                ...
                Case "chkDAU_ESTADO"
                    vObjetoChk.pValorDefault = EchkDAU_ESTADO.pValorDefault
            End Select
        ...
        Handles chkPER_ESTADO.CheckedChanged, chkDAU_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkPER_ESTADO"
                    ...
                    End If
                Case "chkDAU_ESTADO"
                    If EchkDAU_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkDAU_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "DataGridView"
        Private Sub ConfigurarDataGridView()
            ...
            EdgvDetalle.pArrayCamposPkDetalle(1) = "cPVE_ID"
                    ...
                    Select Case e.KeyCode
                        Case Keys.F1
                            If EtxtPVE_ID.pBusqueda Then
                                EtxtPVE_ID.pTexto2 = dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value.ToString
                                MetodoBusquedaDato("", False, EtxtPVE_ID)
                                EtxtPVE_ID.pTexto1 = dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value.ToString
                                EtxtPVE_ID.pTexto2 = dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value.ToString
                            End If
                    End Select
            End Select
        ...
        Handles dgvDetalle.CellDoubleClick
            If dgvDetalle.Columns(dgvDetalle.CurrentCell.ColumnIndex).Name = "cPVE_ID" Then
        ...
        Handles dgvDetalle.CellEnter
            Select Case sender.Columns(dgvDetalle.CurrentCell.ColumnIndex).Name.ToString
                Case "cPVE_ID"
                    'If dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value Is Nothing Then
                    'Else
                    EtxtPVE_ID.pTexto1 = dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value '.ToString
                    'End If
            End Select
        ...
        Handles dgvDetalle.CellValidated
            Select Case sender.Columns(dgvDetalle.CurrentCell.ColumnIndex).Name.ToString
                Case "cPVE_ID"
                    EtxtPVE_ID.pTexto2 = dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value '.ToString
                    ValidarDatos(EtxtPVE_ID, dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value, True, sender.Columns(dgvDetalle.CurrentCell.ColumnIndex).Name.ToString)
                    EtxtPVE_ID.pTexto1 = dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value '.ToString
                    EtxtPVE_ID.pTexto2 = dgvDetalle.Item(dgvDetalle.CurrentCell.ColumnIndex, dgvDetalle.CurrentRow.Index).Value ' .ToString
            End Select
        End Sub
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            ...
            FormatearCampos(txtUSU_ID, "USU_ID", EtxtUSU_ID)
...
#Region "TextBox"
        ...
        Handles txtUSU_ID.GotFocus, txtPER_ID.GotFocus
            Select Case sender.name.ToString
                Case "txtUSU_ID"
        ...
        Handles txtUSU_ID.LostFocus, txtPER_ID.LostFocus
            Select Case sender.name.ToString
                Case "txtUSU_ID"
        ...
        Handles txtDAU_ID.Validated, _
                txtUSU_ID.Validated, _
                txtPER_ID.Validated
            Select Case sender.name.ToString
                Case "txtDAU_ID"
        ...
        Handles txtDAU_ID.KeyPress, _
                txtUSU_ID.KeyPress, _
                txtPER_ID.KeyPress
            Select Case sender.name.ToString
                Case "txtDAU_ID"
        ...
        Handles txtUSU_ID.DoubleClick, txtPER_ID.DoubleClick
            Select Case sender.name.ToString
                Case "txtUSU_ID"
                    oDoubleClick(EtxtUSU_ID, txtUSU_ID, "frmUsuarios")
                Case "txtPER_ID"
        ...
        Handles txtUSU_ID.KeyDown, txtPER_ID.KeyDown
            Select Case e.KeyCode
                Case Keys.F1
                    Select Case sender.name.ToString
                        Case "txtUSU_ID"
