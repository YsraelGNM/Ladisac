...
Namespace Ladisac.Maestros.Views
    Public Class frmMoneda
        ...
        <Dependency()> _
        Public Property IBCMoneda As Ladisac.BL.IBCMoneda
        ...
        Private EchkMON_ORIGEN As New chk
        Private EtxtMON_ID As New txt

        Private Simple As New Ladisac.BE.Moneda
        Private ErrorData As New Ladisac.BE.Moneda.ErrorData
        ...
        Private Sub LimpiarDatos()
            ...
            chkMON_ORIGEN.Checked = Nothing
            chkMON_ORIGEN.CheckState = EchkMON_ORIGEN.pValorDefault
            ErrorProvider1.SetError(chkMON_ORIGEN, Nothing)
        ...
        Private Sub HabilitarNuevo()
            txtMON_ID.Enabled = True
        End Sub
        Private Sub ValoresDefaultNuevo()
            ColocarValoresDefault(chkMON_ORIGEN)
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtMON_ID)
        End Sub
        ...
        Private Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "EliminarRegistro"
                    Simple.MON_ID = txtMON_ID.Text.Trim
                    CodigoId = txtMON_ID.Text.Trim
        ...
        Private Function Ingresar() As Boolean
            ...
            End If
            Simple = Nothing
            Simple = New Ladisac.BE.Moneda
            Return Ingresar
        End Function
        Private Sub DatosCabecera()
            Dim vMON_ID As String = ""
            vMON_ID = Strings.Trim(txtMON_ID.Text)
            Simple.MON_ID = vMON_ID
            Simple.MON_DESCRIPCION = Strings.Trim(txtMON_DESCRIPCION.Text)
            Simple.MON_SIMBOLO = Strings.Trim(txtMON_SIMBOLO.Text)
            Simple.MON_ORIGEN = DevolverTiposCampos(chkMON_ORIGEN)
            Simple.USU_ID = SessionService.UserId
            Simple.MON_FEC_GRAB = Now
            Simple.MON_ESTADO = DevolverTiposCampos(chkMON_ESTADO)
            Simple.MON_CONTROL = 1
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                ...
                Case "chkMON_ORIGEN"
                    Simple.CampoId = EchkMON_ORIGEN.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtMON_DESCRIPCION, Simple.VerificarDatos("MON_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtMON_SIMBOLO, Simple.VerificarDatos("MON_SIMBOLO"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkMON_ORIGEN, Simple.VerificarDatos("MON_ORIGEN"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(lblTitle, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(lblTitle, Simple.VerificarDatos("MON_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkMON_ESTADO, Simple.VerificarDatos("MON_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Function InsertarDatos() As Boolean
            ...
            Simple.MarkAsAdded()
            vRespuestaLocal = Me.IBCMoneda.MantenimientoMoneda(Simple)
        ...
        Private Function ActualizarDatos() As Boolean
            ...
            ActualizarDatos = (Me.IBCMoneda.MantenimientoMoneda(Simple) > 0)
        End Function
        ...
        Private Function EliminarRegistro() As Boolean
            ...
            If (Me.IBCMoneda.MantenimientoMoneda(Simple) > 0) Then
        ...
        Private Sub frm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            ...
            Try
                ...
                FormatearCampos(txtMON_SIMBOLO, "MON_SIMBOLO")
        ...
        Private Sub ConfigurarCheck()
            ...
            EchkMON_ESTADO.pValorDefault = CheckState.Checked
            EchkMON_ESTADO.vEstado0 = ""
            EchkMON_ESTADO.vEstado1 = ""
            EchkMON_ESTADO.vEstadoX = ""
            ConfigurarCheck_Refrescar(EchkMON_ESTADO)

            EchkMON_ORIGEN.pFormatearTexto = True
            EchkMON_ORIGEN.pNombreCampo = "MON_ORIGEN"
            EchkMON_ORIGEN.pSimple = Simple
            EchkMON_ORIGEN.pValorDefault = CheckState.Checked
            EchkMON_ORIGEN.vEstado0 = ""
            EchkMON_ORIGEN.vEstado1 = ""
            EchkMON_ORIGEN.vEstadoX = ""
            ConfigurarCheck_Refrescar(EchkMON_ORIGEN)
        End Sub
        ...
        Public Sub Check_Refrescar()
            InicializarTexto(EchkMON_ORIGEN)
        ...
        Private Sub ConfigurarText()
            ...
            EtxtMON_ID.pBusqueda = False
            EtxtMON_ID.pCadenaFiltrado = ""

            EtxtMON_ID.pOOrm = Nothing
            EtxtMON_ID.pFormularioConsulta = Nothing

            EtxtMON_ID.pComportamiento = Nothing
        ...
        Private Sub chk_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles chkMON_ESTADO.CheckedChanged, _
                    chkMON_ORIGEN.CheckedChanged
            Select Case sender.name.ToString
                Case "chkMON_ESTADO"
                    ...
                    End If
                Case "chkMON_ORIGEN"
                    If EchkMON_ORIGEN.pFormatearTexto Then
                        InicializarTexto(EchkMON_ORIGEN)
                    End If
            End Select
        End Sub
