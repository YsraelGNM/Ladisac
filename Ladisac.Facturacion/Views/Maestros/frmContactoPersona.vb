...
Namespace Ladisac.Maestros.Views
    Public Class frmContactoPersona
#Region "Primaria 1"
        <Dependency()> _
...
#Region "Secundaria 1"
        ...
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCContactoPersona

        ' Controlar la clave de la tabla
        Public vCodigoCOP_ID As String = "" ' COP_ID

        ' CheckBox
        Private EchkPER_ESTADO As New chk
        Private EchkCOP_ESTADO As New chk

        ' ComboBox
        Private EcboCOP_TIPO As New cbo

        ' TextBox
        '' PK
        Private EtxtPER_ID As New txt
        Private EtxtCOP_ID As New txt
        '' Texto
        Private EtxtCOP_DESCRIPCION As New txt
        Private EtxtCOP_DIRECCION As New txt
        Private EtxtCOP_TELEFONO As New txt
        Private EtxtCOP_EMAIL As New txt

        '' Número

        Private Simple As New Ladisac.BE.ContactoPersona
        Private Simple1 As New Ladisac.BE.Personas
        Private ErrorData As New Ladisac.BE.ContactoPersona.ErrorData
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            ...
            FormatearCampos(txtCOP_ID, "COP_ID")
            FormatearCampos(cboCOP_TIPO, "COP_TIPO")
            FormatearCampos(txtCOP_DESCRIPCION, "COP_DESCRIPCION", False)
            FormatearCampos(txtCOP_DIRECCION, "COP_DIRECCION", False)
            FormatearCampos(txtCOP_TELEFONO, "COP_TELEFONO")
            FormatearCampos(txtCOP_EMAIL, "COP_EMAIL", False)
        End Sub
        Private Sub LimpiarDatos()
            InicializarValores(txtPER_ID, ErrorProvider1)
            InicializarValores(txtPER_DESCRIPCION, ErrorProvider1)
            InicializarValores(txtCOP_ID, ErrorProvider1)
            InicializarValores(cboCOP_TIPO, ErrorProvider1)
            InicializarValores(txtCOP_DESCRIPCION, ErrorProvider1)
            InicializarValores(txtCOP_DIRECCION, ErrorProvider1)
            InicializarValores(txtCOP_TELEFONO, ErrorProvider1)
            InicializarValores(txtCOP_EMAIL, ErrorProvider1)
            InicializarValores(chkCOP_ESTADO, ErrorProvider1, False, False, EchkCOP_ESTADO.pValorDefault)
            ColocarValoresDefault(chkCOP_ESTADO)
        End Sub
        Public Sub InicializarValores(ByRef sender As System.Object, _
            ...
            Select Case sender.GetType
                ...
                Case (GetType(System.Windows.Forms.ComboBox))
                    sender.text = BCVariablesNames.TipoContacto.TipoContacto0
        ...
        Private Sub HabilitarNuevo()
            ...
            'txtCOP_ID.Enabled = True
            txtPER_ID.ReadOnly = False
            'txtCOP_ID.ReadOnly = False
        End Sub
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                ...
                Case "chkCOP_ESTADO"
                    vObjetoChk.pValorDefault = EchkCOP_ESTADO.pValorDefault
            End Select
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "RegistroNoEncontrado"
                    ...
                    Simple.COP_ID = txtCOP_ID.Text.Trim
                ...
                Case "EliminarRegistro"
                    ...
                    Simple.COP_ID = txtCOP_ID.Text.Trim
                    vCodigoCOP_ID = txtCOP_ID.Text.Trim
        ...
        '        Case "EliminarRegistro"
        '            Simple.PER_ID = txtPER_ID.Text.Trim
        '            Simple.TDP_ID = txtCOP_ID.Text.Trim
        '            CodigoId = txtPER_ID.Text.Trim
        '            vCodigoTDP_ID = txtCOP_ID.Text.Trim
        '        Case "InicializarDatos"
        '            Simple.PER_ID = txtPER_ID.Text.Trim
        '            Simple.TDP_ID = txtCOP_ID.Text.Trim
        '            CodigoId = txtPER_ID.Text.Trim
        ...
        Private Function Ingresar() As Boolean
            ...
            End If
            'Simple = Nothing
            'Simple = New Ladisac.BE.ContactoPersona
            Return Ingresar
        End Function

        Private Sub DatosCabecera()
            Simple.PER_ID = Strings.Trim(txtPER_ID.Text)
            Simple.COP_ID = Strings.Trim(txtCOP_ID.Text)
            Simple.COP_TIPO = DevolverTiposCampos("COP_TIPO", cboCOP_TIPO.Text, Simple)
            Simple.COP_DESCRIPCION = Strings.Trim(txtCOP_DESCRIPCION.Text)
            Simple.COP_DIRECCION = Strings.Trim(txtCOP_DIRECCION.Text)
            Simple.COP_TELEFONO = Strings.Trim(txtCOP_TELEFONO.Text)
            Simple.COP_EMAIL = Strings.Trim(txtCOP_EMAIL.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.COP_FEC_GRAB = Now
            Simple.COP_ESTADO = DevolverTiposCampos(chkCOP_ESTADO)
        End Sub
        'Private Sub DatosCabecera()
        '    Simple.PER_ID = Strings.Trim(txtPER_ID.Text)
        '    Simple.TDP_ID = Strings.Trim(txtCOP_ID.Text)

        '    If IsNumeric(txtCOP_TELEFONO.Text) Then
        '        Simple.DOP_NUMERO = CDec(txtCOP_TELEFONO.Text.Trim)
        '    Else
        '        txtCOP_TELEFONO.Text = "0"
        '        Simple.DOP_NUMERO = CDec(txtCOP_TELEFONO.Text.Trim)
        '    End If

        '    Simple.USU_ID = SessionService.UserId
        '    Simple.DOP_FEC_GRAB = Now
        '    Simple.DOP_ESTADO = DevolverTiposCampos(chkCOP_ESTADO)
        'End Sub
        ...
        Protected Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                ...
                Case "chkCOP_ESTADO"
                    Simple.CampoId = EchkCOP_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtCOP_ID, Simple.VerificarDatos("COP_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(cboCOP_TIPO, Simple.VerificarDatos("COP_TIPO"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCOP_DESCRIPCION, Simple.VerificarDatos("COP_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCOP_DIRECCION, Simple.VerificarDatos("COP_DIRECCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCOP_TELEFONO, Simple.VerificarDatos("COP_TELEFONO"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCOP_EMAIL, Simple.VerificarDatos("COP_EMAIL"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("COP_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkCOP_ESTADO, Simple.VerificarDatos("COP_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        ...
        '        Case "Cabecera"
        '            resp.rM = Simple.ColocarErrores(txtPER_ID, Simple.VerificarDatos("PER_ID"), ErrorProvider1)
        '            resp.rM = Simple.ColocarErrores(txtCOP_ID, Simple.VerificarDatos("TDP_ID"), ErrorProvider1)
        '            resp.rM = Simple.ColocarErrores(txtCOP_TELEFONO, Simple.VerificarDatos("TDOP_NUMERO"), ErrorProvider1)
        ...
        '            resp.rM = Simple.ColocarErrores(chkCOP_ESTADO, Simple.VerificarDatos("DOP_ESTADO"), ErrorProvider1)
        '    End Select
        '    If Simple.DOP_NUMERO.Length <> LongitudCampoNumero Then
        '        ErrorProvider1.SetError(txtCOP_TELEFONO, "La longitud del campo debe ser: " & LongitudCampoNumero)
        '        vrO = False
        '    End If
        '    Return vrO
        'End Function
        ...
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                ...
                If Me.ActiveControl.Name.ToString = "txtCOP_ID" Then EtxtCOP_ID.pTexto2 = txtCOP_ID.Text
        ...
        Private Sub frm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            ...
            Try
                ...
                'FormatearCampos(txtCOP_ID, "TDP_ID")
        ...
        Private Sub ConfigurarCheck()
            ...
            EchkCOP_ESTADO = EchkTemporal
            EchkCOP_ESTADO.pNombreCampo = "COP_ESTADO"
            EchkCOP_ESTADO.pValorDefault = CheckState.Checked
            ConfigurarCheck_Refrescar(EchkCOP_ESTADO)
        ...
        Public Sub Check_Refrescar()
            ...
            InicializarTexto(EchkCOP_ESTADO)
        End Sub
        ...
        Private Sub ConfigurarText()
            ...
            EtxtCOP_ID = EtxtTemporal
            EtxtCOP_DESCRIPCION = EtxtTemporal
            EtxtCOP_DIRECCION = EtxtTemporal
            EtxtCOP_TELEFONO = EtxtTemporal
            EtxtCOP_EMAIL = EtxtTemporal
        ...
        Private Sub ConfigurarComboBox()
            EcboCOP_TIPO.pNombreCampo = "COP_TIPO"
            cboCOP_TIPO.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList
        End Sub

        Private Sub AdicionarElementoCombosEdicion()
            BuscarFormatos(EcboCOP_TIPO, Simple, cboCOP_TIPO, 0)
        End Sub
        ...
        Private Sub chk_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles chkPER_ESTADO.CheckedChanged, chkCOP_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkPER_ESTADO"
                    ...
                    End If
                Case "chkCOP_ESTADO"
                    If EchkCOP_ESTADO.pFormatearTexto Then
                        InicializarTexto(EchkCOP_ESTADO)
                    End If
            End Select
        End Sub
        ...
        Private Sub Txt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
            Handles txtPER_ID.LostFocus, txtCOP_ID.LostFocus
            Select Case sender.name.ToString
                Case "txtPER_ID"
        ...
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
            Handles txtPER_ID.KeyPress, txtCOP_ID.KeyPress
            Select Case sender.name.ToString
                Case "txtPER_ID"
        ...
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles txtPER_ID.DoubleClick, txtCOP_ID.DoubleClick
            Select Case sender.name.ToString
                Case "txtPER_ID"
                    If Not txtPER_ID.ReadOnly Then
                        EtxtPER_ID.pTexto2 = txtPER_ID.Text
            ...
            Select Case Metodo
                Case "PrimerRegistro"
                    ProcesarNavegar = Me.IBCBusqueda.PrimerRegistro(Simple.CampoPrincipal & "+" & Simple.CampoPrincipalSecundario, _
                                                         Simple.cTabla.NombreLargo)
                Case "RegistroAnterior"
                    Simple.CampoPrincipalValor = CodigoId & vCodigoCOP_ID
