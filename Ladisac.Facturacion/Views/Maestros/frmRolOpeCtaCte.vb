            ...
            Try
                ...
                Select Case vProceso
                    ...
                    Case "PrepararEliminar"
                        ...
                        Dim resp = Me.IBCBusqueda.RegistroAnterior("cast(" & Simple.CampoPrincipal & " as varchar) + " & _
                                                                   "cast( isnull(" & Simple.CampoPrincipalSecundario & ",'') as varchar) +" & _
                                                                   "cast( isnull(" & Simple.CampoPrincipalTercero & ",'') as varchar)", _
                                                                   Simple.CampoPrincipalValor, _
                                                                   Simple.cTabla.NombreLargo)
...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCRolOpeCtaCte

        ' Controlar la clave de la tabla
        Public vCodigoTDO_ID As String = ""
        Public vCodigoDTD_ID As String = ""

        ' CheckBox
        Private Echkroc_EsContable As New chk
        Private EchkROC_ESTADO As New chk
        ...
        Private Etxtcuc_IdMN As New txt
        Private Etxtcuc_IdME As New txt
        '' Texto

        '' Número

        Private Simple As New Ladisac.BE.RolOpeCtaCte
        Private Simple1 As New Ladisac.BE.CtaCte
        Private Simple2 As New Ladisac.BE.DetalleTipoDocumentos
        Private Simple3 As New Ladisac.BE.CuentasContables
        Private ErrorData As New Ladisac.BE.RolOpeCtaCte.ErrorData
        ...
        Private Sub LimpiarDatos()
            ...
            InicializarValores(txtCUC_IDMN, ErrorProvider1)
            InicializarValores(txtCUC_DESCRIPCION_MN, ErrorProvider1)
            InicializarValores(txtCUC_IDME, ErrorProvider1)
            InicializarValores(txtCUC_DESCRIPCION_ME, ErrorProvider1)
            InicializarValores(chkROC_ESCONTABLE, ErrorProvider1, False, False, Echkroc_EsContable.pValorDefault)
            ColocarValoresDefault(chkROC_ESCONTABLE)
            InicializarValores(chkROC_ESTADO, ErrorProvider1, False, False, EchkROC_ESTADO.pValorDefault)
            ColocarValoresDefault(chkROC_ESTADO)
        End Sub
        Private Sub HabilitarNuevo()
            txtCCT_ID.Enabled = True
        ...
        Private Sub CrearCodigoId()
            'ProcesoCrearCodigoId("CrearCodigoSimple", txtCCT_ID)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtCCT_ID.ReadOnly = False
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                Case "CancelarEdicion"
                    CodigoId = CodigoId & vCodigoTDO_ID & vCodigoDTD_ID
                    If Trim(CodigoId) = "" Then CodigoId = "%"
                Case "PrepararEliminar"
                    Simple.Vista = "RegistroAnterior"
                    Simple.CCT_ID = CodigoId
                    Simple.TDO_ID = vCodigoTDO_ID
                ...
                Case "RegistroNoEncontrado"
                    Simple.CCT_ID = txtCCT_ID.Text.Trim
                ...
                Case "EliminarRegistro"
                    Simple.CCT_ID = txtCCT_ID.Text.Trim
                    CodigoId = txtCCT_ID.Text.Trim

                    Simple.TDO_ID = txtTDO_ID.Text.Trim
                    vCodigoTDO_ID = txtTDO_ID.Text.Trim
        ...
        Private Sub DatosCabecera()
            ...
            If Strings.Trim(txtcuc_IdMN.Text) = "" Then
                Simple.cuc_IdMN = Nothing
            Else
                Simple.cuc_IdMN = Strings.Trim(txtcuc_IdMN.Text)
            End If
            If Strings.Trim(txtcuc_IdME.Text) = "" Then
                Simple.cuc_IdME = Nothing
            Else
                Simple.cuc_IdME = Strings.Trim(txtcuc_IdME.Text)
            End If
            Simple.roc_EsContable = DevolverTiposCampos(chkroc_EsContable)
            Simple.USU_ID = SessionService.UserId
            Simple.ROC_FEC_GRAB = Now
            Simple.ROC_ESTADO = DevolverTiposCampos(chkROC_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtcuc_IdMN, Simple.VerificarDatos("cuc_IdMN"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtcuc_IdME, Simple.VerificarDatos("cuc_IdME"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkroc_EsContable, Simple.VerificarDatos("roc_EsContable"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("ROC_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkROC_ESTADO, Simple.VerificarDatos("ROC_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.RolOpeCtaCte
        End Sub
        ...
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                ...
                If Me.ActiveControl.Name.ToString = "txtCUC_IDMN" Then Etxtcuc_IdMN.pTexto2 = Me.ActiveControl.Text
                If Me.ActiveControl.Name.ToString = "txtCUC_IDME" Then Etxtcuc_IdME.pTexto2 = Me.ActiveControl.Text
        ...
        Private Sub NombresFormulariosControles()
            ...
            EtxtCCT_ID.pOOrm = Simple1
            EtxtCCT_ID.pComportamiento = 1
            EtxtCCT_ID.pOrdenBusqueda = 0

            EtxtDTD_ID.pOOrm = Simple2
            EtxtDTD_ID.pComportamiento = 2
            EtxtDTD_ID.pOrdenBusqueda = 0

            Etxtcuc_IdMN.pOOrm = Simple3
            Etxtcuc_IdMN.pComportamiento = 3
            Etxtcuc_IdMN.pOrdenBusqueda = 0

            Etxtcuc_IdME.pOOrm = Simple3
            Etxtcuc_IdME.pComportamiento = 4
            Etxtcuc_IdME.pOrdenBusqueda = 0
...
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            Echkroc_EsContable = EchkTemporal
            Echkroc_EsContable.pNombreCampo = "ROC_ESCONTABLE"
            ConfigurarCheck_Refrescar(Echkroc_EsContable)

            EchkROC_ESTADO = EchkTemporal
            EchkROC_ESTADO.pNombreCampo = "ROC_ESTADO"
            EchkROC_ESTADO.pValorDefault = CheckState.Checked
            ConfigurarCheck_Refrescar(EchkROC_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkROC_ESCONTABLE"
                    Simple.CampoId = Echkroc_EsContable.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkROC_ESTADO"
                    Simple.CampoId = EchkROC_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                Case "chkROC_ESCONTABLE"
                    vObjetoChk.pValorDefault = Echkroc_EsContable.pValorDefault
                Case "chkROC_ESTADO"
                    vObjetoChk.pValorDefault = EchkROC_ESTADO.pValorDefault
            End Select
        ...
        Handles chkROC_ESCONTABLE.CheckedChanged, chkROC_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkROC_ESCONTABLE"
                    If Echkroc_EsContable.pFormatearTexto Then
                        InicializarTextoCheck(Echkroc_EsContable)
                    End If
                Case "chkROC_ESTADO"
                    If EchkROC_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkROC_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "TextBox"
        Private Sub ConfigurarText()
            ...
            EtxtCUC_IDMN = EtxtTemporal
            Etxtcuc_IdME = EtxtTemporal
            ...
            Etxtcuc_IdMN.pBusqueda = True
            Etxtcuc_IdMN.pFormularioConsulta = True

            Etxtcuc_IdME.pBusqueda = True
            Etxtcuc_IdME.pFormularioConsulta = True
            ...
            Select Case e.KeyCode
                Case Keys.F1
                    Select Case sender.name.ToString
                        ...
                        Case "txtCUC_IDMN"
                            TeclaF1(Etxtcuc_IdMN, txtCUC_IDMN)
                        Case "txtCUC_IDME"
                            TeclaF1(Etxtcuc_IdME, txtCUC_IDME)
                    End Select
            End Select
        End Sub
#End Region
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            ...
            FormatearCampos(txtCUC_IDMN, "CUC_IDMN", Etxtcuc_IdMN)
            FormatearCampos(txtCUC_IDME, "CUC_IDME", Etxtcuc_IdME)
        End Sub
        ...
        Private Sub FiltrarOrm()
            ...
            Etxtcuc_IdMN.pOOrm.CadenaFiltrado = ""
            Etxtcuc_IdME.pOOrm.CadenaFiltrado = ""
        End Sub
#Region "ComboBox"
...
#Region "TextBox"
        ...
        txtCUC_IDMN.GotFocus, _
        txtCUC_IDME.GotFocus
            ...
            Select Case sender.name.ToString
                ...
                Case "txtCUC_IDMN"
                    Etxtcuc_IdMN.pTexto1 = sender.text
                Case "txtCUC_IDME"
                    Etxtcuc_IdME.pTexto1 = sender.text
            End Select
        End Sub
        ...
        txtCUC_IDMN.LostFocus, _
        txtCUC_IDME.LostFocus
            ...
            Select Case sender.name.ToString
                ...
                Case "txtCUC_IDMN"
                    Etxtcuc_IdMN.pTexto2 = sender.text
                Case "txtCUC_IDME"
                    Etxtcuc_IdME.pTexto2 = sender.text
            End Select
        End Sub
        Private Sub txt_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtCCT_ID.Validated, _
        txtTDO_ID.Validated, _
        txtDTD_ID.Validated, _
        txtCUC_IDMN.Validated, _
        txtCUC_IDME.Validated
        ...
        txtCUC_IDME.KeyPress
            Select Case sender.name.ToString
                ...
                Case "txtCUC_IDMN"
                    oKeyPress(Etxtcuc_IdMN, e)
                Case "txtCUC_IDME"
                    oKeyPress(Etxtcuc_IdME, e)
            End Select
        End Sub
        ...
        txtCUC_IDMN.DoubleClick, _
        txtCUC_IDME.DoubleClick

            Select Case sender.name.ToString
                'Case "txtTDO_ID"
                '   oDoubleClick(EtxtTDO_ID, txtTDO_ID, "")
                Case "txtCCT_ID"
                    oDoubleClick(EtxtCCT_ID, txtCCT_ID, "")
                Case "txtDTD_ID"
                    oDoubleClick(EtxtDTD_ID, txtDTD_ID, "frmTipoDocumentos")
                Case "txtCUC_IDMN"
                    oDoubleClick(Etxtcuc_IdMN, txtCUC_IDMN, "")
                Case "txtCUC_IDME"
                    oDoubleClick(Etxtcuc_IdME, txtCUC_IDME, "")
            End Select
        End Sub
