...
Namespace Ladisac.Maestros.Views
    Public Class frmDistrito
        ...
        <Dependency()> _
        Public Property IBCDistrito As Ladisac.BL.IBCDistrito
        ...
        Private EchkPRO_ESTADO As New chk
        Private EchkDEP_ESTADO As New chk
        Private EchkPAI_ESTADO As New chk
        Private EtxtPRO_ID As New txt

        Private Simple As New Ladisac.BE.Distrito
        Private ErrorData As New Ladisac.BE.Distrito.ErrorData
        ...
        Private Sub LimpiarDatos()
            ...
            txtPRO_ID.Text = ""
            ErrorProvider1.SetError(txtPRO_ID, Nothing)

            txtPRO_DESCRIPCION.Text = ""
            ErrorProvider1.SetError(txtPRO_DESCRIPCION, Nothing)

            chkPRO_ESTADO.Checked = Nothing
            chkPRO_ESTADO.CheckState = EchkPRO_ESTADO.pValorDefault
            ErrorProvider1.SetError(chkPRO_ESTADO, Nothing)
            ...
            txtDIS_COD_SUNAT.Text = ""
            ErrorProvider1.SetError(txtDIS_COD_SUNAT, Nothing)

            chkDIS_ESTADO.Checked = True
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                ...
                Case "chkPRO_ESTADO"
                    vObjetoChk.pValorDefault = EchkPRO_ESTADO.pValorDefault
                Case "chkDEP_ESTADO"
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtDIS_ID)
        End Sub
        ...
        Private Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "EliminarRegistro"
                    Simple.DIS_ID = txtDIS_ID.Text.Trim
                    CodigoId = txtDIS_ID.Text.Trim
        ...
        Private Function Ingresar() As Boolean
            ...
            End If
            Simple = Nothing
            Simple = New Ladisac.BE.Distrito
            Return Ingresar
        End Function
        Private Sub DatosCabecera()
            Simple.DIS_ID = Strings.Trim(txtDIS_ID.Text)
            Simple.DIS_DESCRIPCION = Strings.Trim(txtDIS_DESCRIPCION.Text)
            Simple.PRO_ID = Strings.Trim(txtPRO_ID.Text)
            Simple.DIS_COD_SUNAT = Strings.Trim(txtDIS_COD_SUNAT.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.DIS_FEC_GRAB = Now
            Simple.DIS_ESTADO = DevolverTiposCampos(chkDIS_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                ...
                Case "chkPRO_ESTADO"
                    Simple.CampoId = EchkPRO_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkDEP_ESTADO"
        ...
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtDIS_DESCRIPCION, Simple.VerificarDatos("DIS_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtPRO_ID, Simple.VerificarDatos("PRO_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(lblTitle, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtDIS_COD_SUNAT, Simple.VerificarDatos("DIS_COD_SUNAT"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(lblTitle, Simple.VerificarDatos("DIS_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkDIS_ESTADO, Simple.VerificarDatos("DIS_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Function InsertarDatos() As Boolean
            ...
            Simple.MarkAsAdded()
            vRespuestaLocal = Me.IBCDistrito.MantenimientoDistrito(Simple)
        ...
        Private Function ActualizarDatos() As Boolean
            ...
            ActualizarDatos = (Me.IBCDistrito.MantenimientoDistrito(Simple) > 0)
        End Function
        ...
        Private Function EliminarRegistro() As Boolean
            ...
            If (Me.IBCDistrito.MantenimientoDistrito(Simple) > 0) Then
        ...
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                If Me.ActiveControl.Name.ToString = "txtPRO_ID" Then EtxtPRO_ID.pTexto2 = Me.ActiveControl.Text
        ...
        Private Sub frm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            ...
            Try
                ...
                FormatearCampos(txtPRO_ID, "PRO_ID")
                FormatearCampos(txtPRO_DESCRIPCION, "PRO_DESCRIPCION")
                FormatearCampos(txtDEP_ID, "DEP_ID")
                FormatearCampos(txtDEP_DESCRIPCION, "DEP_DESCRIPCION")
                FormatearCampos(txtPAI_ID, "PAI_ID")
                FormatearCampos(txtPAI_DESCRIPCION, "PAI_DESCRIPCION")
                FormatearCampos(txtUBIGEO, "UBIGEO")
                FormatearCampos(txtDIS_COD_SUNAT, "DIS_COD_SUNAT")
        ...
        Private Sub ConfigurarCheck()
            ...
            EchkDIS_ESTADO.pValorDefault = CheckState.Checked
            EchkDIS_ESTADO.vEstado0 = ""
            EchkDIS_ESTADO.vEstado1 = ""
            EchkDIS_ESTADO.vEstadoX = ""
            ConfigurarCheck_Refrescar(EchkDIS_ESTADO)

            EchkPRO_ESTADO.pFormatearTexto = True
            EchkPRO_ESTADO.pNombreCampo = "PRO_ESTADO"
            EchkPRO_ESTADO.pSimple = Simple
            EchkPRO_ESTADO.pValorDefault = CheckState.Indeterminate
            EchkPRO_ESTADO.vEstado0 = ""
            EchkPRO_ESTADO.vEstado1 = ""
            EchkPRO_ESTADO.vEstadoX = ""
            ConfigurarCheck_Refrescar(EchkPRO_ESTADO)

            EchkDEP_ESTADO.pFormatearTexto = True
            EchkDEP_ESTADO.pNombreCampo = "DEP_ESTADO"
            EchkDEP_ESTADO.pSimple = Simple
            EchkDEP_ESTADO.pValorDefault = CheckState.Indeterminate
            ...
            EchkPAI_ESTADO.pValorDefault = CheckState.Indeterminate
        ...
        Public Sub Check_Refrescar()
            ...
            InicializarTexto(EchkPRO_ESTADO)
        ...
        Private Sub ConfigurarText()
            EtxtPRO_ID.pTexto1 = ""
            EtxtPRO_ID.pTexto2 = ""
            EtxtPRO_ID.pSoloNumerosDecimales = False
            EtxtPRO_ID.pSoloNumeros = False
            EtxtPRO_ID.pParteEntera = 0
            EtxtPRO_ID.pParteDecimal = 0
            EtxtPRO_ID.pMinusculaMayuscula = True
            EtxtPRO_ID.pBusqueda = True
            EtxtPRO_ID.pCadenaFiltrado = ""

            EtxtPRO_ID.pOOrm = New Ladisac.BE.Provincia
            EtxtPRO_ID.pFormularioConsulta = True

            EtxtPRO_ID.pComportamiento = 1
            EtxtPRO_ID.pOrdenBusqueda = 0
        End Sub
        ...
        Private Sub chk_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles chkPAI_ESTADO.CheckedChanged, _
                    chkDEP_ESTADO.CheckedChanged, _
                    chkPRO_ESTADO.CheckedChanged, _
                    chkDIS_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkDIS_ESTADO"
                    ...
                    End If
                Case "chkPRO_ESTADO"
                    If EchkPRO_ESTADO.pFormatearTexto Then
                        InicializarTexto(EchkPRO_ESTADO)
                    End If
                Case "chkDEP_ESTADO"
        ...
        Private Sub txt_OnGotFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
            Handles txtPRO_ID.GotFocus
            Select Case sender.name.ToString
                Case "txtPRO_ID"
                    EtxtPRO_ID.pTexto1 = sender.text
            End Select
        End Sub
        Private Sub txt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
            Handles txtPRO_ID.LostFocus
            Select Case sender.name.ToString
                Case "txtPRO_ID"
                    EtxtPRO_ID.pTexto2 = sender.text
            End Select
        End Sub
        Private Sub txt_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles txtPRO_ID.Validated
            Select Case sender.name.ToString
                Case "txtPRO_ID"
                    ValidarDatos(EtxtPRO_ID, txtPRO_ID)
            End Select
        End Sub
        Private Sub txt_OnKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) _
            Handles txtPRO_ID.KeyDown
            Select Case sender.name.ToString
                Case "txtPRO_ID"
                    Select Case e.KeyCode
                        Case Keys.F1
                            If EtxtPRO_ID.pBusqueda Then
                                EtxtPRO_ID.pTexto2 = Me.Text
                                ValidarDatos(EtxtPRO_ID, txtPRO_ID)
                                MetodoBusquedaDato("", False, EtxtPRO_ID)
                                EtxtPRO_ID.pTexto1 = Me.Text
                                EtxtPRO_ID.pTexto2 = Me.Text
                            End If
                    End Select
            End Select
        ...
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
            Handles txtPRO_ID.KeyPress
            Select Case sender.name.ToString
                Case "txtPRO_ID"
                    If EtxtPRO_ID.pMinusculaMayuscula Then
                        e.KeyChar = UCase(e.KeyChar)
                    End If
                    If EtxtPRO_ID.pSoloNumerosDecimales Then
                        If Not IsNumeric(e.KeyChar) Then
                            If Asc(e.KeyChar) <> 46 Then
                                ...
                                End If
                            Else
                                If EtxtPRO_ID.pParteDecimal = 0 Then
                                    e.KeyChar = ""
                                End If
                            End If
                        End If
                    End If
                    If EtxtPRO_ID.pSoloNumeros Then
                        If Not IsNumeric(e.KeyChar) Then
                            If Asc(e.KeyChar) <> 8 Then
                                e.KeyChar = ""
                            End If
        ...
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles txtPRO_ID.DoubleClick
            Select Case sender.name.ToString
                Case "txtPRO_ID"
                    EtxtPRO_ID.pTexto2 = txtPRO_ID.Text
                    ValidarDatos(EtxtPRO_ID, txtPRO_ID)
                    If Trim(txtPRO_ID.Text) = "" Then
                        Exit Sub
                    End If
                    If EtxtPRO_ID.pFormularioConsulta Then
                        Dim frmConsulta = Me.ContainerService.Resolve(Of Ladisac.Maestros.Views.frmProvincia)()
                        frmConsulta.DatoBusquedaConsulta = txtPRO_ID.Text
        ...
        Private Sub ValidarDatos(ByRef otxt As txt, ByRef texto As TextBox)
            With otxt
                If .pTexto1 <> .pTexto2 Then
                    .pTexto2 = texto.Text
                    If .pBusqueda Then
                        MetodoBusquedaDato(texto.Text, True, EtxtPRO_ID)
                    End If
                End If
