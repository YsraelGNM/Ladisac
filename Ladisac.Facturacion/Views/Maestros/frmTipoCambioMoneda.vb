...
Namespace Ladisac.Maestros.Views
    Public Class frmTipoCambioMoneda
#Region "Primaria"
        <Dependency()>
...
#End Region
            ...
            Try
                ...
                Select Case vProceso
                    ...
                    Case "PrepararEliminar"
                        ...
                        Simple.CampoPrincipalSecundarioValor = vCodigoMON_ID_0
                        If vCodigoTCA_FECHA = "" Then vCodigoTCA_FECHA = dtpTCA_FECHA.Text
                        Simple.CampoPrincipalTerceroValorString = cMisProcedimientos.FormatoFechaGenerico(vCodigoTCA_FECHA)
                        Dim resp = Me.IBCBusqueda.RegistroAnteriorCompuesto3(Simple.CampoPrincipal, _
                                                                             Simple.CampoPrincipalValor, _
                                                                             Simple.CampoPrincipalSecundario, _
                                                                             Simple.CampoPrincipalSecundarioValor, _
                                                                             Simple.CampoPrincipalTercero, _
                                                                             Simple.CampoPrincipalTerceroValorString, _
                                                                             Simple.cTabla.NombreLargo)
                    ...
                    Case "Load"
                        ...
                        Else
                            OrmBusquedaDatos(vProceso)
                            Dim resp = Me.IBCBusqueda.PrimerRegistroCompuesto3(Simple.CampoPrincipal, _
                                                                               Simple.CampoPrincipalSecundario, _
                                                                               Simple.CampoPrincipalTercero, _
                                                                               Simple.cTabla.NombreLargo)
        ...
        Private Sub NavegarFormulario(ByVal Metodo As String)
            Try
                ...
                Select Case Metodo
                    Case "PrimerRegistro"
                        resp = Me.IBCBusqueda.PrimerRegistroCompuesto3(Simple.CampoPrincipal, _
                                                                       Simple.CampoPrincipalSecundario, _
                                                                       Simple.CampoPrincipalTercero, _
                                                                       Simple.cTabla.NombreLargo)
                    Case "RegistroAnterior"
                        ...
                        Simple.CampoPrincipalSecundarioValor = vCodigoMON_ID_0
                        Simple.CampoPrincipalTerceroValorString = cMisProcedimientos.FormatoFechaGenerico(vCodigoTCA_FECHA)
                        resp = Me.IBCBusqueda.RegistroAnteriorCompuesto3(Simple.CampoPrincipal, _
                                                                         Simple.CampoPrincipalValor, _
                                                                         Simple.CampoPrincipalSecundario, _
                                                                         Simple.CampoPrincipalSecundarioValor, _
                                                                         Simple.CampoPrincipalTercero, _
                    ...
                    Case "RegistroSiguiente"
                        ...
                        Simple.CampoPrincipalSecundarioValor = vCodigoMON_ID_0
                        Simple.CampoPrincipalTerceroValorString = cMisProcedimientos.FormatoFechaGenerico(vCodigoTCA_FECHA)
                        resp = Me.IBCBusqueda.RegistroSiguienteCompuesto3(Simple.CampoPrincipal, _
                                                                          Simple.CampoPrincipalValor, _
                                                                          Simple.CampoPrincipalSecundario, _
                                                                          Simple.CampoPrincipalSecundarioValor, _
                                                                          Simple.CampoPrincipalTercero, _
                                                                          ...
                                                                          Simple.cTabla.NombreLargo)
                    Case "UltimoRegistro"
                        resp = Me.IBCBusqueda.UltimoRegistroCompuesto3(Simple.CampoPrincipal, _
                                                                       Simple.CampoPrincipalSecundario, _
                                                                       Simple.CampoPrincipalTercero, _
                                                                       Simple.cTabla.NombreLargo)
                End Select
...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCTipoCambioMoneda

        <Dependency()> _
        Public Property BCVariablesNames As Ladisac.BL.BCVariablesNames

        ' Controlar la clave de la tabla
        Public vCodigoMON_ID_0 As String = ""
        Public vCodigoTCA_FECHA As String = ""

        ' CheckBox
        Private EchkMON_ESTADO_1 As New chk
        Private EchkMON_ESTADO_0 As New chk
        Private EchkTCA_ESTADO As New chk
        ...
        ' TextBox
        '' PK
        Private EtxtMON_ID_1 As New txt
        Private EtxtMON_ID_0 As New txt
        '' Texto

        '' Número
        Private EtxtTCA_COMPRA As New txt
        Private EtxtTCA_VENTA As New txt

        Private Simple As New Ladisac.BE.TipoCambioMoneda
        Private ErrorData As New Ladisac.BE.TipoCambioMoneda.ErrorData
        Private cMisProcedimientos As New Ladisac.MisProcedimientos


        Private Sub LimpiarDatos()
            InicializarValores(txtMON_ID_1, ErrorProvider1)
            InicializarValores(txtMON_DESCRIPCION_1, ErrorProvider1)
            InicializarValores(txtMON_SIMBOLO_1, ErrorProvider1)
            InicializarValores(txtMON_ORIGEN_1, ErrorProvider1)
            InicializarValores(chkMON_ESTADO_1, ErrorProvider1, False, False, EchkMON_ESTADO_1.pValorDefault)
            ColocarValoresDefault(chkMON_ESTADO_1)
            InicializarTextoCheck(EchkMON_ESTADO_1)
            InicializarValores(txtMON_ID_0, ErrorProvider1)
            InicializarValores(txtMON_DESCRIPCION_0, ErrorProvider1)
            InicializarValores(txtMON_SIMBOLO_0, ErrorProvider1)
            InicializarValores(txtMON_ORIGEN_0, ErrorProvider1)
            InicializarValores(chkMON_ESTADO_0, ErrorProvider1, False, False, EchkMON_ESTADO_0.pValorDefault)
            ColocarValoresDefault(chkMON_ESTADO_0)
            InicializarTextoCheck(EchkMON_ESTADO_0)
            InicializarValores(txtTCA_COMPRA, ErrorProvider1, EtxtTCA_COMPRA.pSoloNumeros, EtxtTCA_COMPRA.pSoloNumerosDecimales)
            InicializarValores(txtTCA_VENTA, ErrorProvider1, EtxtTCA_VENTA.pSoloNumeros, EtxtTCA_VENTA.pSoloNumerosDecimales)
            InicializarValores(chkTCA_ESTADO, ErrorProvider1, False, False, EchkTCA_ESTADO.pValorDefault)
            ColocarValoresDefault(chkTCA_ESTADO)
            InicializarTextoCheck(EchkTCA_ESTADO)
        End Sub
        Private Sub HabilitarNuevo()
            'txtMON_ID_1.Enabled = True
            txtMON_ID_0.Enabled = True
            dtpTCA_FECHA.Enabled = True
        End Sub
        ...
        Private Sub CrearCodigoId()
            'ProcesoCrearCodigoId("CrearCodigoSimple", txtMON_ID_1)
            txtMON_ID_1.Text = BCVariablesNames.MonedaSistema
            MetodoBusquedaDato(txtMON_ID_1.Text, True, EtxtMON_ID_1)
            dtpTCA_FECHA.Value = Now
            'vCodigoTCA_FECHA = dtpTCA_FECHA.Text
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            'txtMON_ID_1.ReadOnly = False
            txtMON_ID_0.ReadOnly = False
        End Sub
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                Case "CancelarEdicion"
                    If vCodigoTCA_FECHA = "" Then vCodigoTCA_FECHA = dtpTCA_FECHA.Text
                    CodigoId = CodigoId & vCodigoMON_ID_0 & cMisProcedimientos.FormatoFechaGenerico(vCodigoTCA_FECHA)
                    If Trim(CodigoId) = "" Then CodigoId = "%"
                Case "PrepararEliminar"
                    Simple.Vista = "RegistroAnterior"
                    Simple.MON_ID_1 = CodigoId
                    Simple.MON_ID_0 = vCodigoMON_ID_0
                    Simple.TCA_FECHA = vCodigoTCA_FECHA
                Case "Load"
                ...
                Case "RegistroNoEncontrado"
                    Simple.MON_ID_1 = txtMON_ID_1.Text.Trim
                    Simple.MON_ID_0 = txtMON_ID_0.Text.Trim
                    Simple.TCA_FECHA = dtpTCA_FECHA.Text.Trim
                Case "NavegarFormulario"
                    Simple.MON_ID_1 = txtMON_ID_1.Text.Trim
                    Simple.MON_ID_0 = txtMON_ID_0.Text.Trim
                    Simple.TCA_FECHA = dtpTCA_FECHA.Text.Trim
                    'CodigoId = CodigoId & vCodigoMON_ID_0 & vCodigoTCA_FECHA
                Case "EliminarRegistro"
                    Simple.MON_ID_1 = txtMON_ID_1.Text.Trim
                    CodigoId = txtMON_ID_1.Text.Trim
                    Simple.MON_ID_0 = txtMON_ID_0.Text.Trim
                    vCodigoMON_ID_0 = txtMON_ID_0.Text.Trim
                    Simple.TCA_FECHA = dtpTCA_FECHA.Text.Trim
                    vCodigoTCA_FECHA = dtpTCA_FECHA.Text.Trim
        ...
        Private Sub DatosCabecera()
            Simple.MON_ID_1 = Strings.Trim(txtMON_ID_1.Text)
            Simple.MON_ID_0 = Strings.Trim(txtMON_ID_0.Text)
            Simple.TCA_FECHA = dtpTCA_FECHA.Value.ToShortDateString
            Simple.TCA_COMPRA = CDbl(txtTCA_COMPRA.Text)
            Simple.TCA_VENTA = CDbl(txtTCA_VENTA.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.TCA_FEC_GRAB = Now
            Simple.TCA_ESTADO = DevolverTiposCampos(chkTCA_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    resp.rM = Simple.ColocarErrores(txtMON_ID_1, Simple.VerificarDatos("MON_ID_1"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtMON_ID_0, Simple.VerificarDatos("MON_ID_0"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(dtpTCA_FECHA, Simple.VerificarDatos("TCA_FECHA"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtTCA_COMPRA, Simple.VerificarDatos("TCA_COMPRA"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtTCA_VENTA, Simple.VerificarDatos("TCA_VENTA"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("TCA_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTCA_ESTADO, Simple.VerificarDatos("TCA_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.TipoCambioMoneda
        End Sub
        Public Sub FiltrarCampos(ByVal vComportamiento As Integer)
        End Sub
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                If Me.ActiveControl.Name.ToString = "txtMON_ID_1" Then EtxtMON_ID_1.pTexto2 = Me.ActiveControl.Text
                If Me.ActiveControl.Name.ToString = "txtMON_ID_0" Then EtxtMON_ID_0.pTexto2 = Me.ActiveControl.Text
        ...
        Private Sub NombresFormulariosControles()
            EtxtMON_ID_1.pOOrm = New Ladisac.BE.Moneda
            EtxtMON_ID_1.pComportamiento = 1
            EtxtMON_ID_1.pOrdenBusqueda = 0

            EtxtMON_ID_0.pOOrm = New Ladisac.BE.Moneda
            EtxtMON_ID_0.pComportamiento = 2
            EtxtMON_ID_0.pOrdenBusqueda = 0
            EtxtMON_ID_0.pOOrm.CadenaFiltrado = " And MON_ID  <>'" & BCVariablesNames.MonedaSistema & "'"
        End Sub
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkMON_ESTADO_1 = EchkTemporal
            EchkMON_ESTADO_1.pNombreCampo = "MON_ESTADO_1"

            EchkMON_ESTADO_0 = EchkTemporal
            EchkMON_ESTADO_0.pNombreCampo = "MON_ESTADO_0"

            EchkTCA_ESTADO = EchkTemporal
            EchkTCA_ESTADO.pValorDefault = CheckState.Checked
            EchkTCA_ESTADO.pNombreCampo = "TCA_ESTADO"

            ConfigurarCheck_Refrescar(EchkMON_ESTADO_1)
            ConfigurarCheck_Refrescar(EchkMON_ESTADO_0)
            ConfigurarCheck_Refrescar(EchkTCA_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkMON_ESTADO_1"
                    Simple.CampoId = EchkMON_ESTADO_1.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkMON_ESTADO_0"
                    Simple.CampoId = EchkMON_ESTADO_0.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkTCA_ESTADO"
                    Simple.CampoId = EchkTCA_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                Case "chkMON_ESTADO_1"
                    vObjetoChk.pValorDefault = EchkMON_ESTADO_1.pValorDefault
                Case "chkMON_ESTADO_0"
                    vObjetoChk.pValorDefault = EchkMON_ESTADO_0.pValorDefault
                Case "chkTCA_ESTADO"
                    vObjetoChk.pValorDefault = EchkTCA_ESTADO.pValorDefault
            End Select
        ...
        Handles chkTCA_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkMON_ESTADO_1"
                    If EchkMON_ESTADO_1.pFormatearTexto Then
                        InicializarTextoCheck(EchkMON_ESTADO_1)
                    End If
                Case "chkMON_ESTADO_0"
                    If EchkMON_ESTADO_0.pFormatearTexto Then
                        InicializarTextoCheck(EchkMON_ESTADO_0)
                    End If
                Case "chkTCA_ESTADO"
                    If EchkTCA_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkTCA_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "TextBox"
        Private Sub ConfigurarText()
            ...
            EtxtMON_ID_1 = EtxtTemporal
            EtxtMON_ID_0 = EtxtTemporal
            EtxtTCA_COMPRA = EtxtTemporal
            EtxtTCA_VENTA = EtxtTemporal
            EtxtMON_ID_1.pBusqueda = True
            EtxtMON_ID_1.pFormularioConsulta = True
            EtxtMON_ID_0.pBusqueda = True
            EtxtMON_ID_0.pFormularioConsulta = True

        End Sub
        Private Sub txt_OnKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) _
        Handles txtMON_ID_1.KeyDown, _
                txtMON_ID_0.KeyDown
            Select Case e.KeyCode
                Case Keys.F1
                    Select Case sender.name.ToString
                        Case "txtMON_ID_1"
                            TeclaF1(EtxtMON_ID_1, txtMON_ID_1)
                        Case "txtMON_ID_0"
                            TeclaF1(EtxtMON_ID_0, txtMON_ID_0)
                    End Select
            End Select
        End Sub
#End Region
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            FormatearCampos(txtMON_ID_1, "MON_ID_1", EtxtMON_ID_1)
            FormatearCampos(txtMON_ID_0, "MON_ID_0", EtxtMON_ID_0)
            FormatearCampos(txtTCA_COMPRA, "TCA_COMPRA", EtxtTCA_COMPRA, False)
            FormatearCampos(txtTCA_VENTA, "TCA_VENTA", EtxtTCA_VENTA, False)
        End Sub
        ...
        Private Sub FiltrarOrm()
            'EtxtMON_ID_1.pOOrm.CadenaFiltrado = ""
            'EtxtMON_ID_0.pOOrm.CadenaFiltrado = ""
        End Sub
#Region "ComboBox"
...
#Region "TextBox"
        ...
        Handles txtMON_ID_1.GotFocus, _
                txtMON_ID_0.GotFocus
            Select Case sender.name.ToString
                Case "txtMON_ID_1"
                    EtxtMON_ID_1.pTexto1 = sender.text
                Case "txtMON_ID_0"
                    EtxtMON_ID_0.pTexto1 = sender.text
            End Select
        End Sub
        Private Sub txt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
        Handles txtMON_ID_1.LostFocus, _
                txtMON_ID_0.LostFocus
            Select Case sender.name.ToString
                Case "txtMON_ID_1"
                    EtxtMON_ID_1.pTexto2 = sender.text
                Case "txtMON_ID_0"
                    EtxtMON_ID_0.pTexto2 = sender.text
            End Select
        End Sub
        Private Sub txt_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtMON_ID_1.Validated, _
                txtMON_ID_0.Validated, _
                txtTCA_COMPRA.Validated, _
                txtTCA_VENTA.Validated
            Select Case sender.name.ToString
                Case "txtMON_ID_1"
                    ValidarDatos(EtxtMON_ID_1, txtMON_ID_1)
                Case "txtMON_ID_0"
                    ValidarDatos(EtxtMON_ID_0, txtMON_ID_0)
                Case "txtTCA_COMPRA"
                    ValidarDatos(EtxtTCA_COMPRA, txtTCA_COMPRA)
                Case "txtTCA_VENTA"
                    ValidarDatos(EtxtTCA_VENTA, txtTCA_VENTA)
            End Select
        End Sub
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
        Handles txtMON_ID_1.KeyPress, _
        txtMON_ID_0.KeyPress, _
        txtTCA_COMPRA.KeyPress, _
        txtTCA_VENTA.KeyPress
            Select Case sender.name.ToString
                Case "txtMON_ID_1"
                    oKeyPress(EtxtMON_ID_1, e)
                Case "txtMON_ID_0"
                    oKeyPress(EtxtMON_ID_0, e)
                Case "txtTCA_COMPRA"
                    oKeyPress(EtxtTCA_COMPRA, e)
                Case "txtTCA_VENTA"
                    oKeyPress(EtxtTCA_VENTA, e)
            End Select
        End Sub
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtMON_ID_1.DoubleClick, _
                txtMON_ID_0.DoubleClick
            Select Case sender.name.ToString
                Case "txtMON_ID_1"
                    oDoubleClick(EtxtMON_ID_1, txtMON_ID_1, "frmMoneda")
                Case "txtMON_ID_0"
                    oDoubleClick(EtxtMON_ID_0, txtMON_ID_0, "frmMoneda")
            End Select
        End Sub
