...
Namespace Ladisac.Maestros.Views
    Public Class frmRolAlmacenTipoArticulos
#Region "Primaria"
        <Dependency()>
...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCRolAlmacenTipoArticulos

        'Controlar la clave de la tabla
        Public vCodigoTIP_ID As String = ""

        ' CheckBox
        Private EchkALM_ESTADO As New chk
        Private EchkTIP_ESTADO As New chk
        Private EchkRAT_ESTADO As New chk
        ...
        '' Número
        Private Simple As New Ladisac.BE.RolAlmacenTipoArticulos
        Private Simple1 As New Ladisac.BE.Almacen
        Private Simple2 As New Ladisac.BE.TipoArticulos

        Private ErrorData As New Ladisac.BE.RolAlmacenTipoArticulos.ErrorData

        Private cMisProcedimientos As New Ladisac.MisProcedimientos
        Private Sub LimpiarDatos()
            InicializarValores(txtALM_ID, ErrorProvider1, EtxtALM_ID.pSoloNumeros, EtxtALM_ID.pSoloNumerosDecimales)
            InicializarValores(txtALM_DESCRIPCION, ErrorProvider1)
            InicializarValores(chkALM_ESTADO, ErrorProvider1, False, False, EchkALM_ESTADO.pValorDefault)

            InicializarValores(txtTIP_ID, ErrorProvider1)
            InicializarValores(txtTIP_DESCRIPCION, ErrorProvider1)
            InicializarValores(chkTIP_ESTADO, ErrorProvider1, False, False, EchkTIP_ESTADO.pValorDefault)

            InicializarValores(chkRAT_ESTADO, ErrorProvider1, False, False, EchkRAT_ESTADO.pValorDefault)

            ColocarValoresDefault(chkALM_ESTADO)
            ColocarValoresDefault(chkTIP_ESTADO)
            ColocarValoresDefault(chkRAT_ESTADO)
        End Sub
        Private Sub HabilitarNuevo()
            txtALM_ID.Enabled = True
            txtTIP_ID.Enabled = True
        End Sub
        ...
        Private Sub CrearCodigoId()
            'ProcesoCrearCodigoId("CrearCodigoSimple", txtALM_ID)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtALM_ID.ReadOnly = False
            txtTIP_ID.ReadOnly = False
        End Sub
        ...
        Private Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                Case "RegistroNoEncontrado"
                    CodigoId = txtALM_ID.Text.Trim
                    vCodigoTIP_ID = txtTIP_ID.Text.Trim
                Case "CancelarEdicion"
                    CodigoId = CodigoId & vCodigoTIP_ID
                Case "PrepararEliminar"
                    Simple.Vista = "RegistroAnterior"
                    Simple.ALM_ID = CodigoId
                    Simple.TIP_ID = vCodigoTIP_ID
                Case "Load"
                ...
                Case "EliminarRegistro"
                    Simple.ALM_ID = txtALM_ID.Text.Trim
                    CodigoId = txtALM_ID.Text.Trim
                    Simple.TIP_ID = txtTIP_ID.Text.Trim
                    vCodigoTIP_ID = txtTIP_ID.Text.Trim
        ...
        Private Sub DatosCabecera()
            Simple.ALM_ID = CInt(txtALM_ID.Text)
            Simple.TIP_ID = Strings.Trim(txtTIP_ID.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.RAT_FEC_GRAB = Now
            Simple.RAT_ESTADO = DevolverTiposCampos(chkRAT_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    resp.rM = Simple.ColocarErrores(txtALM_ID, Simple.VerificarDatos("ALM_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtTIP_ID, Simple.VerificarDatos("TIP_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("RAT_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkRAT_ESTADO, Simple.VerificarDatos("RAT_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.RolAlmacenTipoArticulos
        End Sub
        ...
        Private Sub NombresFormulariosControles()
            EtxtALM_ID.pOOrm = Simple1
            EtxtALM_ID.pComportamiento = 1
            EtxtALM_ID.pOrdenBusqueda = 0

            EtxtTIP_ID.pOOrm = Simple2
            EtxtTIP_ID.pComportamiento = 2
...
#Region "CheckBox"
        Private Sub ConfigurarCheck()
            ...
            EchkTIP_ESTADO = EchkTemporal
            EchkTIP_ESTADO.pNombreCampo = "TIP_ESTADO"

            EchkRAT_ESTADO = EchkTemporal
            EchkRAT_ESTADO.pNombreCampo = "RAT_ESTADO"
            EchkRAT_ESTADO.pValorDefault = CheckState.Checked

            ConfigurarCheck_Refrescar(EchkALM_ESTADO)
            ConfigurarCheck_Refrescar(EchkTIP_ESTADO)
            ConfigurarCheck_Refrescar(EchkRAT_ESTADO)
        End Sub
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                Case "chkALM_ESTADO"
                    Simple.CampoId = EchkALM_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkTIP_ESTADO"
                    Simple.CampoId = EchkTIP_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkRAT_ESTADO"
                    Simple.CampoId = EchkRAT_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                ...
                Case "chkTIP_ESTADO"
                    vObjetoChk.pValorDefault = EchkTIP_ESTADO.pValorDefault
                Case "chkRAT_ESTADO"
                    vObjetoChk.pValorDefault = EchkRAT_ESTADO.pValorDefault
            End Select
        ...
        Handles chkALM_ESTADO.CheckedChanged, chkTIP_ESTADO.CheckedChanged, chkRAT_ESTADO.CheckedChanged
            Select Case sender.name.ToString
                Case "chkALM_ESTADO"
                    ...
                    End If
                Case "chkTIP_ESTADO"
                    If EchkTIP_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkTIP_ESTADO)
                    End If
                Case "chkRAT_ESTADO"
                    If EchkRAT_ESTADO.pFormatearTexto Then
                        InicializarTextoCheck(EchkRAT_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            ...
            FormatearCampos(txtTIP_ID, "TIP_ID", EtxtTIP_ID)
        End Sub
        ...
        Private Sub FiltrarOrm()
            EtxtTIP_ID.pOOrm.CadenaFiltrado = ""
            EtxtALM_ID.pOOrm.CadenaFiltrado = ""
        End Sub
#Region "ComboBox"
...
#Region "TextBox"
        ...
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            Handles txtTIP_ID.DoubleClick, _
                    txtALM_ID.DoubleClick
            Select Case sender.name.ToString
                Case "txtTIP_ID"
                    oDoubleClick(EtxtTIP_ID, txtTIP_ID, "")
                Case "txtALM_ID"
                    oDoubleClick(EtxtALM_ID, txtALM_ID, "")
            End Select
        End Sub
