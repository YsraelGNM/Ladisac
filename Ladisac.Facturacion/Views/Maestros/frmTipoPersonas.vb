...
#Region "Secundaria 1"
        ...
        <Dependency()> _
        Public Property IBCTipoPersonas As Ladisac.BL.IBCTipoPersonas
        ...
        Private EchkCOM_ESTADO As New chk
        Private EchkTPE_ESTADO As New chk

        ' Texto
        Private EtxtTPE_ID As New txt
        Private EtxtTPE_DESCRIPCION As New txt
        Private EtxtCOM_DESCRIPCION As New txt

        ' PK
        Private EtxtCOM_ID As New txt

        Private Simple As New Ladisac.BE.TipoPersonas
        Private ErrorData As New Ladisac.BE.TipoPersonas.ErrorData
...
#Region "Secundaria 2"
        Private Sub LimpiarDatos()
            ...
            InicializarValores(chkTPE_ESTADO, ErrorProvider1, False, False, EchkTPE_ESTADO.pValorDefault)

            InicializarValores(txtCOM_ID, ErrorProvider1)
            InicializarValores(txtCOM_DESCRIPCION, ErrorProvider1)
            InicializarValores(chkCOM_ESTADO, ErrorProvider1, False, False, EchkCOM_ESTADO.pValorDefault)

            InicializarValores(chkTPE_CLIENTE, ErrorProvider1, False, False, EchkTPE_CLIENTE.pValorDefault)
        ...
        Private Sub CrearCodigoId()
            ProcesoCrearCodigoId("CrearCodigoSimple", txtTPE_ID)
        End Sub
        ...
        Private Function Ingresar() As Boolean
            ...
            End If
            Simple = Nothing
            Simple = New Ladisac.BE.TipoPersonas
            Return Ingresar
        End Function

        Private Sub DatosCabecera()
            Simple.TPE_ID = Strings.Trim(txtTPE_ID.Text)
            Simple.TPE_DESCRIPCION = Strings.Trim(txtTPE_DESCRIPCION.Text)
            If Strings.Trim(txtCOM_ID.Text) = "" Then
                Simple.COM_ID = Nothing
            Else
                Simple.COM_ID = Strings.Trim(txtCOM_ID.Text)
            End If


            Simple.TPE_CLIENTE = DevolverTiposCampos(chkTPE_CLIENTE)
            Simple.TPE_PROVEEDOR = DevolverTiposCampos(chkTPE_PROVEEDOR)
            Simple.TPE_TRABAJADOR = DevolverTiposCampos(chkTPE_TRABAJADOR)
            Simple.TPE_BANCO = DevolverTiposCampos(chkTPE_BANCO)
            Simple.TPE_GRUPO = DevolverTiposCampos(chkTPE_GRUPO)
            Simple.TPE_CONTACTO = DevolverTiposCampos(chkTPE_CONTACTO)
            Simple.TPE_TRANSPORTISTA = DevolverTiposCampos(chkTPE_TRANSPORTISTA)

            Simple.USU_ID = SessionService.UserId
            Simple.TPE_FEC_GRAB = Now
            Simple.TPE_ESTADO = DevolverTiposCampos(chkTPE_ESTADO)
        End Sub
        ...
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtTPE_DESCRIPCION, Simple.VerificarDatos("TPE_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(txtCOM_ID, Simple.VerificarDatos("COM_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTPE_CLIENTE, Simple.VerificarDatos("TPE_CLIENTE"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTPE_PROVEEDOR, Simple.VerificarDatos("TPE_PROVEEDOR"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTPE_TRABAJADOR, Simple.VerificarDatos("TPE_TRABAJADOR"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTPE_BANCO, Simple.VerificarDatos("TPE_BANCO"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTPE_GRUPO, Simple.VerificarDatos("TPE_GRUPO"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTPE_CONTACTO, Simple.VerificarDatos("TPE_CONTACTO"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTPE_TRANSPORTISTA, Simple.VerificarDatos("TPE_TRANSPORTISTA"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(lblTitle, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("TPE_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTPE_ESTADO, Simple.VerificarDatos("TPE_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Function InsertarDatos() As Boolean
            ...
            Simple.MarkAsAdded()
            vRespuestaLocal = Me.IBCTipoPersonas.MantenimientoTipoPersonas(Simple)
        ...
        Private Function ActualizarDatos() As Boolean
            ...
            ActualizarDatos = (Me.IBCTipoPersonas.MantenimientoTipoPersonas(Simple) > 0)
        End Function
        ...
        Private Function EliminarRegistro() As Boolean
            ...
            If (Me.IBCTipoPersonas.MantenimientoTipoPersonas(Simple) > 0) Then
        ...
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                If Me.ActiveControl.Name.ToString = "txtCOM_ID" Then EtxtCOM_ID.pTexto2 = Me.ActiveControl.Text
        ...
        Private Sub frm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            ...
            Try
                ...
                FormatearCampos(txtTPE_DESCRIPCION, "TPE_DESCRIPCION", EtxtTPE_DESCRIPCION)
                FormatearCampos(txtCOM_ID, "COM_ID", EtxtCOM_ID)
        ...
        Public Sub Check_Refrescar()
            ...
            InicializarTexto(EchkCOM_ESTADO)
            InicializarTexto(EchkTPE_ESTADO)
        End Sub
        Private Sub ConfigurarCheck()
            ...
            EchkCOM_ESTADO = EchkTemporal
            EchkCOM_ESTADO.pNombreCampo = "COM_ESTADO"
            ConfigurarCheck_Refrescar(EchkCOM_ESTADO)

            EchkTPE_ESTADO = EchkTemporal
            EchkTPE_ESTADO.pNombreCampo = "TPE_ESTADO"
            EchkTPE_ESTADO.pValorDefault = CheckState.Checked
            ConfigurarCheck_Refrescar(EchkTPE_ESTADO)
        End Sub
        Private Sub ValoresDefaultNuevo()
            EchkTPE_CLIENTE.pValorDefault = CheckState.Checked
            ...
            ColocarValoresDefault(chkCOM_ESTADO)
            ColocarValoresDefault(chkTPE_ESTADO)

            EchkTPE_CLIENTE.pValorDefault = CheckState.Unchecked
        End Sub
        Public Sub ColocarValoresDefault(ByRef vObjeto As CheckBox)
            ...
            Select Case vObjeto.Name
                ...
                Case "chkCOM_ESTADO"
                    vObjetoChk.pValorDefault = EchkCOM_ESTADO.pValorDefault
                Case "chkTPE_ESTADO"
                    vObjetoChk.pValorDefault = EchkTPE_ESTADO.pValorDefault
            End Select
        ...
        Private Function DevolverTiposCampos(ByRef oObjeto As CheckBox) As String
            Select Case oObjeto.Name.ToString
                ...
                Case "chkTPE_TRANSPORTISTA"
                    ...
                    Simple.Dato = oObjeto.Text
                Case "chkCOM_ESTADO"
                    Simple.CampoId = EchkCOM_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
                Case "chkTPE_ESTADO"
                    Simple.CampoId = EchkTPE_ESTADO.pNombreCampo
                    Simple.Dato = oObjeto.Text
            End Select
        ...
        Private Sub ConfigurarText()
            ...
            EtxtCOM_ID = EtxtTemporal

            EtxtCOM_ID.pOOrm = New Ladisac.BE.Comision
            EtxtCOM_ID.pComportamiento = 1
        End Sub
...
#Region "Check"
        ...
        Handles chkTPE_CLIENTE.CheckedChanged, _
                chkTPE_PROVEEDOR.CheckedChanged, _
                chkTPE_TRABAJADOR.CheckedChanged, _
                chkTPE_BANCO.CheckedChanged, _
                chkTPE_GRUPO.CheckedChanged, _
                chkTPE_CONTACTO.CheckedChanged, _
                chkTPE_TRANSPORTISTA.CheckedChanged, _
                chkCOM_ESTADO.CheckedChanged, _
                chkTPE_ESTADO.CheckedChanged

            Select Case sender.name.ToString
                Case "chkTPE_CLIENTE"
                    If EchkTPE_CLIENTE.pFormatearTexto Then
                        InicializarTexto(EchkTPE_CLIENTE)
                    End If
                Case "chkTPE_PROVEEDOR"
                    If EchkTPE_PROVEEDOR.pFormatearTexto Then
                        InicializarTexto(EchkTPE_PROVEEDOR)
                    End If
                Case "chkTPE_TRABAJADOR"
                    If EchkTPE_TRABAJADOR.pFormatearTexto Then
                        InicializarTexto(EchkTPE_TRABAJADOR)
                    End If
                Case "chkTPE_BANCO"
                    If EchkTPE_BANCO.pFormatearTexto Then
                        InicializarTexto(EchkTPE_BANCO)
                    End If
                Case "chkTPE_GRUPO"
                    If EchkTPE_GRUPO.pFormatearTexto Then
                        InicializarTexto(EchkTPE_GRUPO)
                    End If
                Case "chkTPE_CONTACTO"
                    If EchkTPE_CONTACTO.pFormatearTexto Then
                        InicializarTexto(EchkTPE_CONTACTO)
                    End If
                Case "chkTPE_TRANSPORTISTA"
                    If EchkTPE_TRANSPORTISTA.pFormatearTexto Then
                        InicializarTexto(EchkTPE_TRANSPORTISTA)
                    End If
                Case "chkCOM_ESTADO"
                    If EchkCOM_ESTADO.pFormatearTexto Then
                        InicializarTexto(EchkCOM_ESTADO)
                    End If
                Case "chkTPE_ESTADO"
                    If EchkTPE_ESTADO.pFormatearTexto Then
                        InicializarTexto(EchkTPE_ESTADO)
                    End If
            End Select
        End Sub
...
#Region "Text"
        Private Sub txt_OnKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) _
            Handles txtTPE_ID.KeyDown, txtTPE_DESCRIPCION.KeyDown, txtCOM_ID.KeyDown

            Select Case e.KeyCode
                Case Keys.F1
                    Select Case sender.name.ToString
                        Case "txtCOM_ID"
                            TeclaF1(EtxtCOM_ID, txtCOM_ID)
                    End Select
            End Select
        End Sub
