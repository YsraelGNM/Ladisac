...
#Region "Secundaria"
        <Dependency()> _
        Public Property IBC As Ladisac.BL.IBCTipoUnidad
        ...
        Private Simple As New Ladisac.BE.TipoUnidad
        Private ErrorData As New Ladisac.BE.TipoUnidad.ErrorData
        ...
        Private Sub LimpiarDatos()
            ...
            ColocarValoresDefault(chkTUN_ESTADO)
        End Sub
        Private Sub HabilitarNuevo()
            txtTUN_ID.Enabled = True
        End Sub
        ...
        Private Sub CrearCodigoId()
            'ProcesoCrearCodigoId("CrearCodigoSimple", txtTUN_ID)
        End Sub
        Private Sub HabilitarEscrituraNuevo()
            txtTUN_ID.ReadOnly = False
        End Sub
        ...
        Public Sub OrmBusquedaDatos(ByVal vProceso As String)
            Select Case vProceso
                ...
                Case "PrepararEliminar"
                    ...
                    Simple.TUN_ID = CodigoId
                Case "Load"
                    Simple.Vista = "PrimerAnterior"
                    Simple.TUN_ID = CodigoId
                Case "RegistroNoEncontrado"
                    Simple.TUN_ID = txtTUN_ID.Text.Trim
                Case "NavegarFormulario"
                    CodigoId = CodigoId
                Case "EliminarRegistro"
                    Simple.TUN_ID = txtTUN_ID.Text.Trim
                    CodigoId = txtTUN_ID.Text.Trim
        ...
        Private Sub DatosCabecera()
            ...
            Simple.TUN_DESCRIPCION = Strings.Trim(txtTUN_DESCRIPCION.Text)
            Simple.USU_ID = SessionService.UserId
            Simple.TUN_FEC_GRAB = Now
            Simple.TUN_ESTADO = DevolverTiposCampos(chkTUN_ESTADO)
        End Sub
        Private Function Validar(ByVal vModelos As String) As Boolean
            ...
            Select Case vModelos
                Case "Cabecera"
                    ...
                    resp.rM = Simple.ColocarErrores(txtTUN_DESCRIPCION, Simple.VerificarDatos("TUN_DESCRIPCION"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(pnCuerpo, Simple.VerificarDatos("USU_ID"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(btnImagen, Simple.VerificarDatos("TUN_FEC_GRAB"), ErrorProvider1)
                    resp.rM = Simple.ColocarErrores(chkTUN_ESTADO, Simple.VerificarDatos("TUN_ESTADO"), ErrorProvider1)
            End Select
            Return vrO
        End Function
        Private Sub InicializarOrm()
            Simple = Nothing
            Simple = New Ladisac.BE.TipoUnidad
        End Sub
...
#Region "CheckBox"
        ...
        Handles chkTUN_ESTADO.CheckedChanged
...
#Region "Secundaria 2"
        Private Sub FormatearCampos()
            ...
            FormatearCampos(txtTUN_DESCRIPCION, "TUN_DESCRIPCION", EtxtTUN_DESCRIPCION)
        End Sub
...
#Region "TextBox"
        ...
        txtTUN_DESCRIPCION.Validated
            Select Case sender.name.ToString
                Case "txtTUN_ID"
                    ValidarDatos(EtxtTUN_ID, txtTUN_ID)
                Case "txtTUN_DESCRIPCION"
                    ValidarDatos(EtxtTUN_DESCRIPCION, txtTUN_DESCRIPCION)
            End Select
        End Sub
        Private Sub txt_OnKeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
        Handles txtTUN_ID.KeyPress, _
        txtTUN_DESCRIPCION.KeyPress
            Select Case sender.name.ToString
                Case "txtTUN_ID"
                    oKeyPress(EtxtTUN_ID, e)
                Case "txtTUN_DESCRIPCION"
                    oKeyPress(EtxtTUN_DESCRIPCION, e)
            End Select
        End Sub
