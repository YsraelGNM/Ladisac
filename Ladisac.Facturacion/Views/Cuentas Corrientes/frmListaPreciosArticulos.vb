...
Namespace Ladisac.CuentasCorrientes.Views
    Public Class frmListaPreciosArticulos
#Region "Primaria"
        <Dependency()>
...
#Region "Secundaria"
        ...
        Private EtxtLPR_ID_ADJ As New txt
        ...
        Private CompuestoObjeto2 As New Ladisac.BE.DetalleListaPrecios
        ...
        Private Sub LimpiarDatos()
            ...
            txtLPR_ID_ADJ.Text = ""
            ErrorProvider1.SetError(txtLPR_ID_ADJ, Nothing)

            txtLPR_DESCRIPCION_ADJ.Text = ""
            ErrorProvider1.SetError(txtLPR_DESCRIPCION_ADJ, Nothing)

            txtPER_ID_ADJ.Text = ""
            ErrorProvider1.SetError(txtPER_ID_ADJ, Nothing)

            txtPER_DESCRIPCION_ADJ.Text = ""
            ErrorProvider1.SetError(txtPER_DESCRIPCION_ADJ, Nothing)

            txtPER_LINEA_CREDITO.Text = "0.00"
            txtDeuda.Text = "0.00"
        ...
        Private Sub DatosCabecera()
            ...
            If Strings.Trim(txtLPR_ID_ADJ.Text) = "" Then
                Compuesto.LPR_ID_ADJ = Nothing
            Else
                Compuesto.LPR_ID_ADJ = Strings.Trim(txtLPR_ID_ADJ.Text)
            End If
        ...
        Private Sub ActualizarModelo()
            EtxtLPR_ID_ADJ.pOOrm.CadenaFiltrado = " and LPR_ESTADO='ACTIVO' and Isnull(rtrim(ltrim(PER_ID)),'')<>'' and LPR_PRINCIPAL='" & cboLPR_PRINCIPAL.Text & "' and PER_ID in (SELECT PER_ID FROM	vwRolPersonaTipoPersona WITH (NOLOCK) WHERE	TPE_DESCRIPCION='FERRETERO' and PER_CLIENTE='SI' and TPE_CLIENTE='SI' and PER_ID in (Select PER_ID from	vwPersonas WITH (NOLOCK) where PER_GRUPO='SI'))"
            'txtLPR_ID_ADJ.Text = ""
            'MetodoBusquedaDato(txtLPR_ID_ADJ.Text, True, EtxtLPR_ID_ADJ)
        End Sub
        Protected Overrides Function ProcessDialogKey(ByVal keyData As System.Windows.Forms.Keys) As Boolean
            If keyData = Keys.Enter Then
                ...
                If Me.ActiveControl.Name.ToString = "txtLPR_ID_ADJ" Then EtxtLPR_ID_ADJ.pTexto2 = Me.ActiveControl.Text
        ...
        Private Sub frm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            Try
                ...
                FormatearCampos(txtPER_ID_ADJ, "CODIGO")
                FormatearCampos(txtLPR_ID_ADJ, "LPR_ID")
                FormatearCampos(txtLPR_DESCRIPCION_ADJ, "LPR_DESCRIPCION")
                ...
                Select Case SessionService.UserId
                    Case "GLOPEZ"
                        Compuesto.CadenaFiltrado = " AND PER_ID in (Select PER_ID " & _
                                                                  ...
                                                                  " where PER_CLIENTE='SI' " & _
                                                                        ...
                                                                        " and TPE_DESCRIPCION not in('MEGACENTROS') " & _
                                                                        " and RTP_ESTADO='ACTIVO') " & _
                                                   " OR IsNull(PER_ID,' ') = ' ' "
                        EtxtPER_ID.pOOrm.CadenaFiltrado = " AND Codigo in (Select PER_ID " & _
                                                                         " from vwRolPersonaTipoPersona " & _
                                                                         " where PER_CLIENTE='SI' " & _
                                                                               " and TPE_CLIENTE='SI' " & _
                                                                               " and TPE_DESCRIPCION not in('MEGACENTROS') " & _
                    ...
                    Case "IRMAGxx"
                        Compuesto.CadenaFiltrado = " AND PER_ID in (Select PER_ID " & _
                                                                  ...
                                                                  " where PER_CLIENTE='SI' " & _
                                                                        " and TPE_CLIENTE='SI' " & _
                                                                        " and TPE_DESCRIPCION not in ('FERRETERO','DISTRIBUIDOR') " & _
                                                                        " and RTP_ESTADO='ACTIVO') "
                        EtxtPER_ID.pOOrm.CadenaFiltrado = " AND Codigo in (Select PER_ID " & _
                                                                         " from vwRolPersonaTipoPersona " & _
                                                                         " where PER_CLIENTE='SI' " & _
                                                                               ...
                                                                               " and TPE_DESCRIPCION not in ('FERRETERO','DISTRIBUIDOR') " & _
                                                                               " and RTP_ESTADO='ACTIVO') "
                    Case "ADMIN", "EYLENR", "JGONZALES", "IRMAG"
                        Compuesto.CadenaFiltrado = "  "
                    Case Else
                        Compuesto.CadenaFiltrado = " AND PER_ID in (Select PER_ID " & _
                                                   " from vwRolPersonaTipoPersona " & _
                                                   " where PER_CLIENTE='SIx' " & _
                                                   " and TPE_CLIENTE='SIx' " & _
                                                   " and TPE_DESCRIPCION in ('FERRETEROx','DISTRIBUIDORx','MEGACENTROSx') " & _
                                                   " and RTP_ESTADO='ACTIVOx') " & _
                                                   " OR  IsNull(PER_ID,'')='xx' "
                End Select
            Catch ex As Exception
        ...
        Private Sub NombresFormulariosControles()
            ...
            EtxtPER_ID_CLI.pComportamiento = 4
            EtxtPER_ID_CLI.pOrdenBusqueda = 1
            EtxtPER_ID_CLI.pBusqueda = False

            EtxtLPR_ID_ADJ.pOOrm = New Ladisac.BE.ListaPreciosArticulos
            EtxtLPR_ID_ADJ.pComportamiento = 5
            EtxtLPR_ID_ADJ.pOrdenBusqueda = 0
...
#Region "TextBox"
        Private Sub ConfigurarText()
            ...
            EtxtPER_ID_CLI.pTexto1 = ""
            EtxtPER_ID_CLI.pTexto2 = ""
            EtxtPER_ID_CLI.pSoloNumerosDecimales = False
            EtxtPER_ID_CLI.pSoloNumeros = False
            EtxtPER_ID_CLI.pParteEntera = 0
            EtxtPER_ID_CLI.pParteDecimal = 0
            EtxtPER_ID_CLI.pMinusculaMayuscula = True
            EtxtPER_ID_CLI.pBusqueda = True
            EtxtPER_ID_CLI.pCadenaFiltrado = ""

            EtxtLPR_ID_ADJ.pTexto1 = ""
            EtxtLPR_ID_ADJ.pTexto2 = ""
            EtxtLPR_ID_ADJ.pSoloNumerosDecimales = False
            EtxtLPR_ID_ADJ.pSoloNumeros = False
            EtxtLPR_ID_ADJ.pParteEntera = 0
            EtxtLPR_ID_ADJ.pParteDecimal = 0
            EtxtLPR_ID_ADJ.pMinusculaMayuscula = True
            EtxtLPR_ID_ADJ.pBusqueda = True
            EtxtLPR_ID_ADJ.pCadenaFiltrado = ""
            ...
            EtxtLPR_ID_ADJ.pOOrm = New Ladisac.BE.ListaPreciosArticulos
            EtxtLPR_ID_ADJ.pFormularioConsulta = True
...
#Region "TextBox"
        ...
        Handles txtPER_ID.GotFocus, txtMON_ID.GotFocus, txtPER_ID_ADJ.GotFocus, txtLPR_ID_ADJ.GotFocus
            Select Case sender.name.ToString
                Case "txtPER_ID"
                ...
                Case "txtLPR_ID_ADJ"
                    EtxtLPR_ID_ADJ.pTexto1 = sender.text
            End Select
        End Sub
        Private Sub txt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) _
        Handles txtPER_ID.LostFocus, txtMON_ID.LostFocus, txtPER_ID_ADJ.LostFocus, txtLPR_ID_ADJ.LostFocus
            Select Case sender.name.ToString
                Case "txtPER_ID"
                ...
                Case "txtLPR_ID_ADJ"
                    EtxtLPR_ID_ADJ.pTexto2 = sender.text
            End Select
        End Sub
        Private Sub txt_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtPER_ID.Validated, txtMON_ID.Validated, txtPER_ID_ADJ.Validated, txtLPR_ID_ADJ.Validated
            Select Case sender.name.ToString
                Case "txtPER_ID"
                ...
                Case "txtPER_ID_CLI"
                    txtPER_ID_ADJ.Text = cMisProcedimientos.VerificarLongitud(txtPER_ID_ADJ.Text, txtPER_ID_ADJ.MaxLength)
                    ValidarDatos(EtxtPER_ID_CLI, txtPER_ID_ADJ)
                Case "txtLPR_ID_ADJ"
                    txtLPR_ID_ADJ.Text = cMisProcedimientos.VerificarLongitud(txtLPR_ID_ADJ.Text, txtLPR_ID_ADJ.MaxLength)
                    ValidarDatos(EtxtLPR_ID_ADJ, txtLPR_ID_ADJ)
                    'ActualizarNombreListaPrecio()
                    'Enlazar()
            End Select
        End Sub
        ...
        Handles txtPER_ID.KeyPress, txtMON_ID.KeyPress, txtPER_ID_ADJ.KeyPress, txtLPR_ID_ADJ.KeyPress
            Select Case sender.name.ToString
                Case "txtPER_ID"
                ...
                Case "txtLPR_ID_ADJ"
                    oKeyPress(EtxtLPR_ID_ADJ, e)
            End Select
        End Sub
        Private Sub txt_OnDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles txtPER_ID.DoubleClick, txtMON_ID.DoubleClick, txtPER_ID_ADJ.DoubleClick
            Select Case sender.name.ToString
                Case "txtPER_ID"
                ...
                Case "txtPER_ID_CLI"
                    oDoubleClick(EtxtPER_ID_CLI, txtPER_ID_ADJ, "frmPersonas")
            End Select
        End Sub
        ...
        Handles txtPER_ID.KeyDown, txtMON_ID.KeyDown, txtPER_ID_ADJ.KeyDown, txtLPR_ID_ADJ.KeyDown
            Select Case e.KeyCode
                Case Keys.F1
                    Select Case sender.name.ToString
                        Case "txtPER_ID"
                        ...
                        Case "txtPER_ID_CLI"
                            TeclaF1(EtxtPER_ID_CLI, txtPER_ID_ADJ)
                        Case "txtLPR_ID_ADJ"
                            TeclaF1(EtxtLPR_ID_ADJ, txtLPR_ID_ADJ)
                    End Select
            End Select
        End Sub
#End Region
...
#End Region
        ...
        Private Sub cboLPR_PRINCIPAL_SelectedValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboLPR_PRINCIPAL.SelectedValueChanged
            ActualizarNombreListaPrecio()
            If (txtPER_ID.Text.Trim) <> "" Then dgvDetalle.Rows.Clear()
            ProcesarGridVacio()
        End Sub
        ...
        Private Sub SaldosCliente()
            Try
                txtPER_LINEA_CREDITO.Text = Me.IBCBusqueda.LineaCreditoPersona(txtPER_ID_ADJ.Text)
                Dim ds As New DataSet
                dgvSaldos.DataSource = Nothing
                If txtPER_ID_ADJ.Text.Trim <> "" Then
                    Dim sr As New StringReader(IBCMaestro.EjecutarVista(DA.SPNames.spSaldosClienteXML, _
                                                                        DBNull.Value, _
                                                                        DBNull.Value, _
                                                                        txtPER_ID_ADJ.Text.Trim, _
                                                                        " "))
                    Dim vcontrol As Int16 = sr.Peek
                    If vcontrol <> -1 Then
                        ds.ReadXml(sr)
        ...
        Private Sub btnEnlazar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEnlazar.Click
            Enlazar()
            ProcesarGridVacio()
        End Sub
        ...
        Private Sub Enlazar()
            Dim CodigoIdAdj As String
            'Compuesto.LPR_ID = txtLPR_ID.Text.Trim
            CompuestoObjeto2.LPR_ID = txtLPR_ID_ADJ.Text.Trim
            CodigoIdAdj = txtLPR_ID_ADJ.Text.Trim

            CompuestoObjeto2.Vista = "ListarRegistros"
            Dim NombreProcedimiento As String = CompuestoObjeto2.SentenciaSqlBusqueda()
            Dim ds As New DataSet
            Dim sr As New StringReader(IBCMaestro.EjecutarVista(NombreProcedimiento, CodigoIdAdj, " "))
            Dim vcontrol As Int16 = sr.Peek
            If vcontrol <> -1 Then
                ds.ReadXml(sr)
                Dim x As Int32 = 0
                ...
                If (ds.Tables(0).Rows.Count > 0) Then
                    While (x < ds.Tables(0).Rows.Count)
                        ...
                        With ds.Tables(0).Rows(x)
                            While y < ds.Tables(0).Columns.Count
                                dgvDetalle.Item(y, dgvDetalle.Rows.Count - 1).Value = Formatos(.Item(y).GetType.ToString, .Item(y).ToString())
                                dgvDetalle.Item("cLPR_ID", dgvDetalle.Rows.Count - 1).Value = txtLPR_ID.Text
                                dgvDetalle.Item("cLPR_DESCRIPCION", dgvDetalle.Rows.Count - 1).Value = txtLPR_DESCRIPCION.Text
                                dgvDetalle.Item("cEstadoRegistro", dgvDetalle.Rows.Count - 1).Value = 0
                                y = y + 1
                            End While
                            y = 0
                        End With
        ...
        Private Sub ProcesarGridVacio(Optional ByVal vControl As Integer = 0)
            If dgvDetalle.RowCount = 0 Then ' Habilita
                If (txtPER_ID.Text.Trim) <> "" Then
                    txtLPR_ID_ADJ.Enabled = True
                    txtLPR_ID_ADJ.ReadOnly = False
                    EtxtLPR_ID_ADJ.pBusqueda = True
                    btnEnlazar.Enabled = True
                End If
            End If
            If dgvDetalle.RowCount >= 1 Then ' Inhabilita
                txtLPR_ID_ADJ.Enabled = False
                txtLPR_ID_ADJ.ReadOnly = True
                EtxtLPR_ID_ADJ.pBusqueda = False
